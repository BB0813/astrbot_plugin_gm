...
| 配置 key | 控制命令 | 说明 |
|------|------|------|
| `title_admins` | `/头衔` | 在名单中的 QQ 可设置/取消群头衔（不受群管身份限制） |
| `group_admin_admins` | `/设管理` `/取消管理` | 在名单中的 QQ 可设/取消群管理（#167 已支持） |
| `kick_admins` | `/踢` | 在名单中的 QQ 可踢出群成员 |

**全局配置示例**（在 AstrBot 配置文件中）：

```json
{
  "group_overrides": {
    "123456789": {
      "title_admins": ["10001", "10002"],
      "group_admin_admins": ["20001"],
      "kick_admins": ["30001"]
    },
    "987654321": {
      "title_admins": ["10003"]
...
```
/设置群配置 group_admin_admins ["20001", "20002"]
/设置群配置 title_admins ["10001"]
```
