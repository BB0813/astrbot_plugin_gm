...
- **元教训**：同一反模式若在多个反思中反复触发（"校验失败整体降级"在 PR #123 + Issue #130 + #131 三轮 + #132 五次 + #140 九次触发），说明反思流程缺乏自动化保护机制，必须固化 Pre-check 短路器到流程而非仅依赖记忆。**建议未来版本将本节作为反思流程的前置硬阻断清单**。

### 4.33 Issue #141 删除 `/取消头衔` 命令模板（owner-driven 减法 + 外部契约删除）

- **场景**：维护者自提删除 `/取消头衔` 命令，属"对外接口收敛"而非 bug。
- **分类与优先级**：`enhancement` + `medium`。**4 条决策路径**：① 横切重写+breaking-change → 最低 medium；② owner-driven 改法明确 → 不升 high；③ 工作量可控局部化 → 不升 high；④ 与 #124/#126 无冲突 → 维持。**关键边界**：命令有外部用户依赖（教程/脚本引用）→ 不降 low；纯内部清理才适用 `low`。
- **必给标签**：`enhancement`(0.95) + `breaking-change`(0.90) + `command`(0.95) + `group-management`(0.85) + `title`(0.80) + `documentation`(0.85) + `cleanup`/`removal`(0.80，建议仓库新建) + `needs-discussion`(0.80) + `interface-removal`(新建) + `migration`(0.70，分支 B/C)；**慎用 `deprecation`**：仅分支 B/C（标记废弃但保留过渡期）适用，纯删除不适用；**慎用 `onebot`**：本次不涉及 OneBot 新增 API 调用，≤0.30。
- **可行性 A/B/C/D**：A 硬删除 5-15 行/0.25-0.5 天；B 删除+引导 30-60 行/0.5-1 天；C 删除+替换 60-100 行/1-2 天；D 分阶段 0.5-1 天观察。九项删除类反向清单 + 七处同步清单（main.py + `_GM_COMMAND_NAMES` + README + 帮助命令 + `_conf_schema.json` + CHANGELOG + 迁移指南）均适用。
- **删除类反向清单 9 项**（参 §4.15）：精确删除路径 + 保留路径边界（grep `set_group_special_title` 调用点） + 调用点清空 + README 帮助文案清理 + yield 帮助文本同步 + schema 语义缩窄 + bot 回复模板措辞清理 + CHANGELOG breaking-change 标注 + 替代方案提示。
- **跨 Issue 互引**：`/取消头衔` 历史上反复失败（#111/#119/#125），应**显式互引**而非仅作背景提及，强化"本次删除源于历史反复失败"的论证。
- **替代方案责任归属**：`needs-discussion` 显式列出："建议维护者在 PR 阶段明确分支 A 是否为最终方案，还是暂存方案待分支 B/C 评估"。

### 4.34 Issue #142 事件驱动型批量撤回模板（仓库第 5 种新模式：动作联动型）

- **场景**：维护者自提"踢人后按配置自动/手动批量撤回被踢用户本群全部历史"（不撤回 >2 分钟超时消息）。属 owner-driven 复合诉求，子诉求 3 个：① 踢人时自动联动 ② 新增配置项 ③ 新增手动指令。
- **分类与优先级**：`enhancement` + `medium`。**5 条决策路径**（新增默认值决策）：① 横切权限 → 最低 medium；② owner-driven + 已标准化模式 → 不升 high；③ 工作量可控 + 迁移路径明确 → 不升 high；④ 跨适配器风险已识别但有兜底 → 维持；⑤ **默认值决策（默认 false → 维持；默认 true → 升 high/silent behavior change）**。
- **`needs-info`/`needs-discussion` 双校准**：owner-driven 缺决策非信息 → `needs-info` ≤0.30 + `needs-discussion` ≥0.80。
- **必给标签**：`enhancement`(0.95) + `recall`(0.95) + `command`(0.95) + `message-history`(0.90) + `group-management`(0.88) + `configuration`(0.85) + `onebot`(0.80-0.85) + `onebot-extension`(0.55-0.65，仓库新建) + `throttling`/`rate-limit`(新建) + `pagination`(新建) + `silent-behavior`(默认 true 时) + `partial-failure`(新建) + `breaking-change`(**按场景 4 档精细化**：新增配置+默认 false ≤0.30 / 新增配置+默认 true 0.55-0.65 / 修改既有默认值 0.70-0.80 / 删除既有 0.90-0.95) + `needs-discussion`(0.85) + `needs-info`(≤0.30) + `event-driven`/`on-kick`(0.60-0.70) + `privacy`(0.55-0.65，仓库新建)。
- **撤回增强类第 5 种角色相关模式（第 5 种敏感性模式）**：事件驱动型（踢出事件 → 批量撤销动作），与 #130 权限重构 / #131 引用消息触发型 / #134/#136 装饰字符 QQ / #140 举报路由并列。
- **事件驱动型必查 10 项**：① 触发源（命令 handler 内 vs OneBot `notice` 事件 hook）② OneBot `notice` 跨适配器字段名差异（`sub_type`/`event`/`notice_type`）③ `message_history` 容量现状与 deque maxlen ④ 撤回 2 分钟硬约束下的降级策略（静默忽略 vs 统计失败数）⑤ 批撤回限速（OneBot 20-30/s）⑥ bot 自身消息排除 ⑦ 撤回≠清除（对其他群员仍可见撤回提示——必须明示）⑧ 配置 schema：`clear_history_on_kick` 全局 + `group_overrides` 覆盖 ⑨ 区分"维护者命令触发" vs "任意踢出事件触发" ⑩ 撤回失败的统计与日志。
- **可行性分支**：A 120-200 行/2-3 天（依赖 PR #123 缓存；未 merge 时翻倍） / B 200-350 行/3-5 天（含 OneBot notice hook 跨适配器）/ C 250-450 行/4-6 天（完整 helper 抽取）/ D 分阶段先 A 后 B/C。
- **`group_overrides` 嵌套结构**：`needs-discussion` 必须列：扁平键覆盖 vs 全量覆盖；跨群生效顺序（全局 → 群覆盖 → 用户）。
- **跨 Issue 决策路径同构互引模板**：两 Issue 决策路径同构时（如 #142 与 #140）应显式互引："本 Issue 与 #140 决策路径同构（横切权限但有兜底 / owner-driven + 已标准化模式 / 工作量可控 + 迁移路径明确 / 风险已识别但有降级），维持 medium 优先级"。
- **缓存骨架集成度必查**：PR #123 `message_history` 是否已 merge、容量、是否启用、跨群隔离语义——**必须查证而非假设**，可行性评估基础依赖此。
- **撤回 ≠ 清除**：分析时必须明示该区分（撤回是"删除消息"语义，对其他群成员而言仍可见撤回提示），否则用户预期与实际行为不符。

### 4.35 Issue #143 联动清除数据类模板（扩展 §4.34 模式 + 沉淀"动作联动型"为仓库第 5 类）

- **场景**：维护者自提"踢人时联动清除该用户聊天记录 + 新增独立命令"，参考外部 `zcj-ui/astrbot_plugin_group_guardian` 仓库实现。属 owner-driven 复合诉求，涉及联动破坏性操作。
- **分类与优先级**：`enhancement` + `medium`。**5 条决策路径**：① 高风险动作（踢人 + 批量撤回）→ 最低 medium；② owner-driven + 已标准化模式 → 不升 high；③ 配置可关闭（默认 false）+ 兜底机制 → 不升 high；④ 跨适配器风险已识别但有降级路径 → 维持；⑤ **破坏性不可逆操作（清除他人聊天记录）→ 维持 medium 且 `needs-discussion` 上调至 0.85+**。
- **必给标签**：`enhancement`(0.95) + `command`(0.85) + `configuration`(0.80) + `group-management`(0.90，**#140 硬约束不可遗漏**) + `recall`(0.85) 或 `message-clear`（仓库新建） + `permission`(0.80) + `message-history`(0.75) + `onebot`(0.85) + `compatibility`(0.75，跨适配器差异矩阵必须给) + `external-reference`(0.85，参考外部仓库) + `kick`(0.80，仓库新建高频场景标签) + `owner-driven`(0.85) + `permission-model`(0.75) + `needs-discussion`(0.80) + `needs-info`(0.20，**#140 双校准硬约束对冲**)。
- **`external-reference` 与 `merge-request` 区别**：参考借鉴外部仓库代码用 `external-reference`(0.85)；真正合入外部代码用 `merge-request`(0.50-0.65)。本 Issue 是前者。
- **外部仓库参考类 Issue 前置检查清单**（扩展"合入外部仓库类"模板）：① 外部仓库许可证兼容性 ② 依赖兼容性（aiohttp/httpx/websocket-client 版本）③ OneBot 标准 API vs 私有扩展 API 使用清单 ④ 维护责任归属（fork vs 复制代码 vs 引入依赖）⑤ 配置 schema 扩展冲突。
- **动作联动型模板 10 项必查**（仓库第 5 种敏感性模式，#143 沉淀）：① 回执一致性（部分失败时如何回执主动作成功状态）② 通知风暴（B 触发群内"撤回消息"提示影响其他群成员）③ 限流策略（B 是高成本动作如何分批）④ 配置粒度（可关闭/可调上限/按群覆盖）⑤ 权限双校验（A 和 B 各自权限如何对齐/bot 自身权限边界）⑥ 时间窗约束（B 是否有协议级时间窗）⑦ 跨适配器差异矩阵（B 依赖 API 的适配器差异）⑧ 本地缓存兜底（B 失败时是否复用既有缓存）⑨ 可观测性（成功 N/M / 跳过 X 条因时间窗 / 失败 Y 条因限流 的分类回执）⑩ 去重（本地缓存与服务端历史可能产生重复 message_id）。
- **可行性分支**：A 120-200 行/2-3 天（含本地缓存复用）/ B 350-500 行/5-7 天（含 server-side 撤回）/ C 400-600 行/7-10 天（通用 helper 抽取）/ D 分阶段先 A 后 B/C。**下限规则**：涉及"本地缓存 + 协议兜底 + 配置 schema + 指令入口"4 件套的最小 A 分支至少 1 天起步。
- **红旗警示（仓库新沉淀）**："owner + 清除数据"是仓库第 5 类敏感性模式，必须显式评估 ① 滥用风险 ② 合规风险（主流平台 Bot API 通常不允许运营方清空他人消息）③ 不可逆性（撤回时间窗外服务端无法恢复）④ 隐私边界（"清理某人存在痕迹"风险）⑤ bot 风控（批量 API 调用可能触发服务端封禁）⑥ 群解散/退群边界（被踢后再次入群历史消息是否仍可撤回）。
- **OneBot 跨适配器 `delete_msg` 矩阵**（建议仓库沉淀，类似 #135/#136 的字段差异矩阵）：NapCat / Lagrange / go-cqhttp × 时间窗内 / 时间窗外 / 批量 / 单条 × 最大返回 / 限速 / 权限边界。
- **`_record_mute_and_maybe_kick` (L870) 自动踢人路径**：是踢人链路高频改动点，建议作为踢人链路统一接入点专门审查，多次触及（#143 自动踢人是否也触发清除又是 `needs-discussion` 决策点）。

### 4.36 5 种角色相关模式仓库全景（#130/#131/#134/#136/#140/#143 联合沉淀）
...
| **5. 动作联动型（#143 新增）** | **#143** | **A 成功后自动触发 B** | **回执一致性/通知风暴/限流/可配置/权限双校验** |

注：#134/#136（装饰字符 QQ）虽然是 bug 而非 enhancement，但其"输入触发 + 跨适配器兼容性 + 解析路径"模式被 #143 联动动作类继承，仍在仓库模式全景中占据独立位置。

### 4.37 反思流程硬约束升级（第 10-11 次违反重复检测措辞禁令）

- **触发统计**（同一反模式连续多 Issue 反复触发）：
  - 重复检测"未检测到重复"/"无重复"措辞禁令：**#134 + #136 + #138 + #139 + #140 + #141 + #142** 已达 **第 10-11 次违反**。
  - `#needs-info` 双校准硬约束缺失：**#140 + #143** 第 9-10 次违反。
  - `parser` 标签误标反模式：**#138 + #143** 第 9-10 次违反。
  - 校验失败整体降级反模式：**#140 + #141 + #142 + #143** 累计第 9-11 次触发。
- **必须升级为反思流程第一优先级 Pre-check 硬阻断**（与"校验失败短路器"并列）：
  1. ⛔ 重复检测措辞扫描：是否写了"未检测到重复/无重复/可能是 #X 重复"？必须改写为"暂未发现"+建议检索关键词+⚠️ 措辞核对。
  2. ⛔ `needs-info` 双校准扫描：owner-driven 但涉及 ≥3 决策点时是否 `needs-info` ≤0.30 + `needs-discussion` ≥0.80 同时给？
  3. ⛔ `parser` 标签语义扫描：本次分析是否涉及装饰字符 QQ / `@` 解析路径？若两者均不涉及 → 删除或归零。
  4. ⛔ 校验失败短路器：参见 §4.32。
- **建议仓库建立"反思流程硬约束清单 v2"**（§4.32 v1 升级版）：包含 5-6 项硬阻断检查，任何一项违规须落笔前修复，不得作为反思输出。

### 4.38 多次反思暴露的系统性问题（#141/#142/#143 联合元层面教训）

- **同源不同症 Issue 互引规范缺失**：`/取消头衔` 历史上反复失败（#111/#119/#125）+ 本次删除（#141）应形成完整证据链，但当前仓库未沉淀"同命令失败案例 → 删除命令动机"的论证模板。建议新增 `command-deprecation-history` 标签或 `command-failure-history.md` 仓库文档。
- **`breaking-change` 置信度精细化（4 档而非 2 档）**（#142 沉淀）：① 新增配置+默认 false ≤0.30 ② 新增配置+默认 true 0.55-0.65（silent behavior change）③ 修改既有默认值 0.70-0.80 ④ 删除既有 0.90-0.95。
- **优先级决策路径升级为 5 条**（#142 沉淀）：新增第 5 条"默认值决策（默认 false → 维持；默认 true → 升 high/silent behavior change）"。
- **外部参考仓库清单维护**：`zcj-ui/astrbot_plugin_group_guardian` 等外部参考源应建立清单（许可证、活跃度、API 兼容性），便于后续同类 Issue 分析。
- **5 种敏感性模式全景**（§4.36）：仓库建议建立 `mode-classification.md` 文档固化各类模式的必查项、必给标签、可行性分支。
- **反思流程纪律性问题**：连续多 Issue 反复触发同一反模式，说明反思提示词缺乏前置硬约束。建议将本反思流程硬约束固化到提示词模板，而非依赖记忆。

### 4.39 Issue 分析与标签经验（高层规则，详见 memory.md）
