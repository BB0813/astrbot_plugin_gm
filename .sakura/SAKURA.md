...
动作 A 成功后触发动作 B（kick→clear-history / ban→notify 等），共享配置与权限。**必查项 14 项**含回执一致性/通知风暴/限流/权限双校验/可观测性分类回执/去重/隐私合规/默认值决策/外部仓库验证/跨适配器撤回权限/消息ID去重/批量撤回时限/审计日志/用户通知。与 PR #123 缓存骨架强关联需显式声明依赖状态。

**外部仓库参考 vs 合入的精细化区分**：
- **参考实现**：仅借鉴思路 → `external-repo` + `reference`(0.80-0.85)
- **直接合入**：合并代码 → `merge-request` + `license-check`(0.90+)
- 两者**不可混用**，必须先访问外部仓库验证实现路径，再判断归属。

**动作联动型必给标签**（#143/#145 教训）：
`group-management`(0.92) + `needs-info`(≤0.30) + `needs-discussion`(0.75-0.80) + `compatibility`(0.55-0.70) + `permission-model`(0.80-0.85) + `auto-action`(0.75-0.80) + `privacy`(0.70-0.80) + `onebot`(0.65-0.70) + `onebot-extension`(0.55-0.65) + `breaking-change`(0.40-0.65) + `throttling`(0.75-0.80) + `notification-storm`(0.80)。

**默认值决策路径**（#145 教训）：涉及用户数据删除时，默认值决策是优先级关键——默认 `false` 维持 medium，默认 `true`（自动清历史）升 high（隐私风险）。

### 3.7 模式七：撤回增强族（#142 自动+手动批量撤回）
...
### 3.8 模式八：校验短路器触发恢复模式（#131-#145 累计触发11+次）
当分析者遇到字段校验失败时，**禁止整体降级为 `other`/`无法评估`/`无建议`**。必须先恢复实质性判断，再修复字段输出格式。**第11类触发红旗**：标签漏项未触发校验失败时应升级为P0自检，与"校验失败短路器"并列。强制自检门槛：输出前扫描"无建议/无法评估/空/未检测到重复"任一关键词，若含则回溯字段层面定位校验失败点，实质性判断维持原始判断。

### 3.9 模式九：隐私合规强制检查（#143/#145 新建）
涉及用户数据删除/修改时，**必须自动提示添加 `privacy` 标签**（0.70-0.85）。清历史类 Issue 需评估：审计日志（消息ID+操作者+时间戳）、用户通知（被踢用户私聊通知）、撤回通知抑制（避免群内风暴）、失败回滚（部分失败时回执N/M）。
