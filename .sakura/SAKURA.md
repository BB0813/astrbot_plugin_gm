...
### 4.24 Issue #132 权限模型重构完整模板（v2 强化版）

- **场景**：维护者自提"移除 plugin_admins，bot 自动检测群原生身份"。属权限模型重构 + 配置项删除（breaking-change）+ 18+ 调用点横切改造。
- **分类 `enhancement` + `breaking-change`（≥0.90）+ `medium`**：维护者本人发起 + 已有标准化模式（§4.17）+ 工作量可控 → 不升 high；横切重写 + 用户已配置数据将失效 → 不降 low。
- **必给标签基线**：`enhancement` + `breaking-change`(0.90) + `deprecation`(0.75) + `permission-model`(0.80, 新建) + `configuration`(0.85) + `group-management`(0.85) + `bot-role`/`sender-role`(0.70, 新建) + `onebot`/`compatibility`(0.65) + `command` + `needs-discussion`(升 0.75)；`needs-info`(≤0.45, owner-driven 决策待确认)；**移除 `question`**(置信度应 ≤0.05)。
- **可行性四段式**（高复用价值模板）：① 已有可复用机制（`has_*_admin_rights`、`get_group_member_info`、`sender.role`）② 必改 N 项（配置读取移除、权限 helper 重写、命令入口分流）③ 风险点（API 失败兜底、缓存策略、跨适配器、配置迁移、权限提升审计、同类配置项扫描）④ 验证清单（按群抽样 × 角色矩阵 × 跨适配器）。
- **工作量估算下限**：18+ 调用点 + 五处同步（main.py / schema / README / 帮助 / CHANGELOG）+ 迁移指南 + 测试验证 → **200-350 行、3-5 天**（不是 150-250 行、1-3 天）。
- **权限提升风险审计必须给具体命令示例**：至少 2-3 个（如 `/踢人`、`/全员禁言`、`/改群名`），并建议"是否需要为这些命令保留 super-admin 概念"作为决策点。
- **同类配置项扫描**：`title_admins`/`group_admin_admins`/`kick_admins` 是否同步迁移，建议一次性重构而非分散迁移。
- **优先级决策路径显式化**（4 条）：① 横切重写 + breaking-change → 最低 medium ② 维护者本人发起 + 已有标准化模式 → 不升 high ③ 工作量可控 + 迁移路径明确 → 不升 high ④ 跨适配器风险已识别但有兜底 → 维持 medium。
- **重复检测前置过滤**：分类差异（bug vs enhancement）是硬过滤条件；分类不同则重复置信度上限 0.3；#132 把 #125 bug 误判为 enhancement 重复（0.95 失真）即反例。
- **精确行号引用**：批量行号（L398-401、L454-468 等）必须前置声明"已通过 main.py 实际读取验证"或"基于项目记忆 + 既有 PR 模式推断，建议 PR 提交前以最新 main.py 行号为准"。

### 4.25 Issue #133 解禁/解除禁言类标准模板（与 #125 头衔类对等）

- **场景**：`/解禁 @用户` 提示成功但目标仍被禁言。与"头衔类"分析模板（§4.5/#125）对等——"提示成功但实际未生效"通用模式的具体落地。
- **分类 `bug` + `medium`**：核心解禁命令局部不可用（最低 medium）+ 误判用户状态风险（不降 low）+ 跨适配器兼容风险已识别但有兜底（不升 high）。
- **必给标签**：`bug` + `command`(0.90) + `group-management`(0.88) + `unmute`/`lift-ban`(0.85, 新建) + `mute-management`(0.80) + `onebot`(0.78) + `compatibility`(0.75) + `bot-role`(0.65) + `needs-info`(0.85-0.90, 缺适配器版本/截图/目标身份是硬阻塞)。
- **必查项 9 项**：① Bot 在该群角色（部分协议要求群主专属解禁）② 目标用户是否仍在群内 ③ OneBot 实现版本（NapCat/Lagrange/go-cqhttp 对 `delete_group_ban` 语义差异）④ `duration` 参数语义（`duration=0` vs 不传 vs `duration=-1`，注意 **falsy 判空会把 0 误判为未传**）⑤ `_extract_at_qq` 解析（必须按 segment/user_id，不按空格 split）⑥ `get_group_member_info` 回读校验（API 返回 ok ≠ 实际生效）⑦ 目标若已退群如何兜底 ⑧ 完整执行日志 ⑨ 最近配置变更。
- **"提示成功但实际未生效"扩展为群管理动作通用模板**（覆盖）：解禁(`set_group_ban` duration) + 头衔(`set_group_special_title` special_title) + 设精(`set_essence_msg`) + 改群名(`set_group_name`) + 全员禁言(`set_group_whole_ban` enable)。
- **"早退语句吞噬共享逻辑"陷阱**：若解禁流程中存在权限早退/参数校验早退导致回读/日志未执行，必须前置检查。
- **"禁言/解禁"语义至少 3 层**：A. 解除群管 API 禁言（`set_group_ban` duration=0，最常见）B. 解除"禁我"自怼状态（插件内部记录）C. 解除审批工作流中"待审批禁言状态"。**必先确认用户指哪一层**。
- **可行性显式分支判定**：分支A（API 层兼容）~20-40 行+1 天；分支B（需补完整状态回读）~50-100 行+1.5-2 天；分支C（需重构解禁入口/权限校验）~100-150 行+2-3 天。
- **重复检测措辞禁令**："可能是 #X 的重复" 明令禁止；改用"暂未发现" + 建议检索关键词 + ⚠️ 措辞核对提示。
- **"默认行为"反模式**：`needs-info` 置信度应根据缺信息关键程度评估（解禁类缺适配器版本/截图是硬阻塞，应 ≥0.85），不是固定的 0.40-0.55。

### 4.26 Issue #134 装饰字符 QQ / 视觉欺骗型用户名解析类（新晋高频模式）

- **场景**：用户用 `@𝓒𝓪𝓷𝓬𝓮𝓻`（花体字/数学字母 Unicode/手写体）等"看起来是 QQ 号但实际是字符串"的输入，bot 命令"成功"但实际行为错误（解禁了字面量而非目标用户）。
- **分类 `bug` + `medium`**：命令"成功"但实际行为错误（bug）+ 核心禁言/解禁命令对群管影响大（medium）；误解禁的安全影响大于误禁言（用户已被禁言状态下误解禁使其重新获得发言权）→ 优先级评估应纳入此权重。
- **必给标签**：`bug`(0.95) + `command`(0.90) + `parser`(0.85) + `at-parse`/`at-extract`(0.80, 新建) + `group-management`(0.75) + `onebot`/`compatibility`(0.65) + `needs-info`(0.55, 缺版本/commit/日志/装饰字符来源) + `mute`/`unmute`(0.70, 新建)。
- **必查项 6 项**：① `_extract_at_qq` 是否 NFKC 归一化（`unicodedata.normalize('NFKC', s)` + 正则 `\d{5,12}` 强校验）② 装饰字符 QQ 经 AstrBot 事件框架后是原始字符串还是已规范化 ③ OneBot v11 `message` 段 `qq` 字段类型（字符串 vs 数字）④ 跨适配器对装饰字符 QQ 的处理 ⑤ 群号是否被装饰字符污染 ⑥ 是否有 `get_group_member_info` 回读校验。
- **修复方向**：在 `_extract_at_qq` 顶部加 `unicodedata.normalize('NFKC', s)` + 数字段提取正则 + 错误提示"目标 QQ 格式不正确"。
- **测试矩阵**：NapCat/Lagrange/go-cqhttp × 数学字母/手写体/Emoji 风格/组合 Unicode 字母 → 至少半天构造测试用例。
- **触发关键词**：`@用户` + `解禁`/`禁言`/`踢人`/`设管` + 装饰字符/数学字母/花体字。
- **与 §头衔类模板的复用**：记忆 §4.5/§4.12 中"`_extract_at_qq` 必查项"应在解禁/禁言/踢人/设管等所有 @ 解析命令中主动激活（不仅头衔类），扩展关键词覆盖范围。
- **工作量下限校准**：小修复（<50 行）也要避免低估为 1 天内——含适配器验证至少 1 天。

### 4.27 反模式反复触发的自动化保护机制（Issue #130-#134 五连击反思）

- **场景统计**：PR #123 第六轮 + Issue #130 + Issue #131 三轮 + Issue #132 两次 + Issue #133 两次 + Issue #134 一次 = **同一根因反复触发 ≥7 次**。
- **核心机制**：反思流程中存在自动化保护机制缺失。"校验失败短路器"必须固化到反思流程 Pre-check 步骤，而非依赖每次人工识别。
- **固化规则（必须写入反思指令）**：若反思摘要提及字段校验错误（如 `expected <REASON>` / `expected <SUGGESTED_TITLE>`），则：
  1. **只修复字段输出格式**（如把"无建议"改成实际建议）
  2. **实质性判断维持原始判断**（分类/可行性/标签/标题、重复检测必须基于 Issue 实质内容独立完成）
  3. **不得连带退化**（禁止把 `bug`/`enhancement` 降级为 `other`、标签留空、可行性写"无法评估"、标题写"无建议"、重复检测写"可能是 #X 的重复"）
- **Pre-check 短路器触发条件**：任何字段出现"无建议/无法评估/空/未检测到重复/可能是#X的重复"前，必须先确认是否源于校验失败——校验失败应**局部修复字段输出格式**。
- **正确输出保底模板**（校验失败时仍须输出）：分类 + 优先级 + 摘要 + 建议标签列表 + 重复检测（"暂未发现" + 关键词 + ⚠️措辞核对）+ 标题改写（"可保留"或轻量规范化）。
- **USERNAME 等保留字校验错误的处置**：`unknown`/`.` 等异常输入不应触发整体降级，应：① 单独标注"作者信息缺失，建议人工补充"；② 对`.`标题进行主动改写；④ 其他字段继续基于 Issue 正文评估。
- **未来防御（记忆幻觉警告）**：引用项目记忆 § 编号前必须确认存在（曾编造"§4.13 UX 增强低估"——§4.13 实际是撤回默认行为/按用户指定编号 Issue #124 而非 UX 估算）。

### 4.28 Issue #130-#134 综合教训与下一步

- **高频模式清单**（必须套用对应模板）：① 权限模型重构（§4.24）② 解禁/解除禁言类（§4.25）③ 装饰字符 QQ 解析（§4.26）④ 加群申请（§4.18）⑤ 群待办（§4.21）⑥ STT 语音（§4.16）⑦ 按群配置（§4.7/§4.8）⑧ 撤回缓存（§4.3/§4.6）⑨ 头衔类（§4.5/§4.12）。
- **新标签建议清单**（仓库可新建）：`at-parse`/`at-extract`、`bot-role`/`sender-role`、`permission-model`、`deprecation`、`reply`/`quote-message`、`bot-capability`、`onebot-extension`、`mute`/`unmute`、`mute-management`、`stt`/`voice`、`join-request`/`join-approval`。
- **流程强化建议**：① 分析开始前先跑一次"模式关键词扫描" ② 即使输出校验失败，实质性判断应先在分析过程中固化 ③ 对每个分析结果做"反模式自检清单" ④ Pre-check 短路器必须在反思指令中前置强调。

## 5. Issue 分析与标签经验（高层规则，详见 memory.md）

- 标题为"。"、"，"或信息极少时必须基于正文错误文本、复现命令和代码线索检索。
- **结构化输出校验失败 ≠ 信息不足**：应**修复字段输出**保留实质判断（分类/可行性/标签/关键问题列表），不得整体退化为"无法评估"——这是仓库反复出现的反模式（PR #123 第六轮审查 + Issue #130-#134 五连击即因此完全失败）。pre-check：任何字段出现"无建议/无法评估/空"前，确认是否源于校验失败。
- 不能因校验失败把 `bug`/`enhancement` 降级为 `other`、标签留空、可行性"无法评估"。
- 标签建议覆盖主类型、模块和风险；至少保留主标签与核心模块；高频模块标签：`recall`、`message-history`、`command`、`parser`、`onebot`、`group-management`、`moderation`、`stt`/`voice`、`title`/`special-title`、`at-parse`、`permission-model`、`bot-role`、`deprecation`、`unmute`/`lift-ban`。
...
- 信息不足时可标 `needs-info`；头衔类/解禁类/装饰字符类 bug 还需 bot 群角色、目标用户角色、近期配置变更；解禁类缺适配器版本/截图是硬阻塞，`needs-info` 应 ≥0.85（非默认 0.40-0.55）。
...
- **重复触发反模式的纪律性**：同一反模式若在多个反思中反复触发（如"校验失败整体降级"在 PR #123 + Issue #130 + #131 + #132 + #133 + #134 ≥7 次触发），说明反思流程缺乏自动化保护机制，必须固化 Pre-check 短路器到流程而非仅依赖记忆。
- **正确输出保底模板**（校验失败时仍须输出）：分类 + 优先级 + 摘要 + 建议标签列表 + 重复检测（"暂未发现"+ 关键词 + ⚠️措辞核对）+ 标题改写（"可保留"或轻量规范化）。
...
- 对群管理"成功提示"保持保守：设/取消头衔、设/取消管理、禁言/解禁、踢人、撤回、改群昵称等都应检查 API 返回与状态，失败时给明确提示；"API 返回 ok ≠ 实际生效"是已知反模式，必要时回读（`get_group_member_info`）；装饰字符 QQ/花体字等视觉欺骗型用户名是 `@用户` 解析类 bug 的高频根因（参 §4.26）；"API 返回 ok ≠ UI 生效"专项警示（QQ 群待办 API 接受但 UI 异步刷新 1-2 秒）。
