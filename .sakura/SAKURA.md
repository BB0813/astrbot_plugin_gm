...
### 4.28 Issue #138 命令别名/重命名决策类 Issue 标准模板（owner-driven 双动作）

- **触发场景**：维护者本人发起，包含**减法**（删除既有命令）+ **改名/别名**（如 `/取消设精` → `/取消群精华`）混合诉求。常见中文 bot 插件命令命名收敛场景。
- **关键洞察纪律**：分析前**必须**扫描 `_GM_COMMAND_NAMES` 元组与所有 `@filter.command(...)` 装饰器，建立"命令名 → handler"映射，避免误判为"全新功能新增"——很多时候新命令名实为既有命令的别名/语义收敛。
- **分类与优先级**：分类 `enhancement` (0.95)；优先级 `medium`（删除既有命令涉及 breaking-change 维持 medium）或 `low`（owner-driven 纯减法 + 无横切关注点可降），**必须显式 4 条决策路径**：① 主动减法+owner-driven → 应降 low ② 但涉及既有命令删除（breaking-change）→ 维持 medium ③ 别名决策未确定 → `needs-discussion` 高权重 ④ 跨适配器风险低（v11 标准）→ 维持 medium。
- **必给标签基线**：`enhancement`(0.95) + `command`(0.95) + `breaking-change`(0.65-0.85，按删除影响面) + `deprecation`(0.65-0.85) + `group-management`(0.85) + `needs-discussion`(0.80-0.90，**核心：作者意图存在二义性需先澄清**) + `documentation`(0.75-0.85) + `needs-info`(≤0.30 owner-driven 缺决策非信息) + **新增** `rename`/`alias`(0.75-0.85，建议仓库新建) + 可选 `onebot`(0.30-0.50)。
- **应避免标签**：`parser`(本类非解析逻辑改动) + `recall`(本类非消息撤回，与 `delete_essence_msg` 无关)。
- **可行性显式分支 A/B/C**：A（最小：仅删除 `/删群公告`，20-30 行 / 0.5-1 天）；B（完整：删除+新增别名+保留双名共存，100-150 行 / 1.5-2.5 天）；C（重命名+迁移指南+配置 schema 调整）。**纯删除也需七处同步**（main.py + `_GM_COMMAND_NAMES` + README + 帮助命令 + CHANGELOG + metadata.yaml 描述 + 测试边界场景）。
- **反向 9 项删除清单**（命令删除专用）：① 精确删除 handler 行号 ② helper 是否被其他路径调用（需全文件搜索 `_delete_group_notice` 等）③ 元组 `_GM_COMMAND_NAMES` 删除项 ④ README 帮助文本/表格清理 ⑤ `/撤回` 等命令 yield 帮助文本同步 ⑥ 帮助命令文案删除 ⑦ CHANGELOG breaking-change 标注 ⑧ 替代方案提示（手动撤回/既有等价命令）⑨ 引用消息解析路径兼容（如有 `_get_reply_id` 依赖）。
- **作者意图二义性处理**：若标题/正文暗示"新增 /取消群精华"，必须先确认：① 是否已存在等价命令（`/取消设精` 已实现 `delete_essence_msg`）② 是否需保留双名共存 ③ 是否需迁移指南。在可行性章节显式给决策树："作者回复 (a) 改名 → 走分支 B/C；(b) 未察觉已存在 → 走分支 A；(c) 不回复 → 默认 A 最保守"。
- **重复检测措辞第 N+1 次红旗**：本次分析命中"未检测到重复"禁用措辞——反思 checklist 必须前置扫描："未检测到重复"/"无重复"/"可能是 #X 的重复"任一命中即红旗。正确措辞："暂未发现重复 Issue，建议检索关键词：`/删群公告` `/取消精华` `notice_id` `群精华取消` `delete_group_notice` `delete_essence_msg` `alias` `rename`"。
- **建议标题**：`[enhancement][medium] 移除 /删群公告 命令并新增 /取消群精华（引用消息触发，待澄清作者意图）`（保留"待澄清"信号）或精简版：`[enhancement][medium] 移除 /删群公告；/取消群精华 已被 /取消设精 覆盖（待确认是否改名）`（更精确反映"等价命令已存在"的关键洞察）。
- **仓库建议新建标签**：`rename`/`alias`（命令重命名/别名专用）+ `cleanup`/`removal`（功能裁剪专用）+ `ui-text`（命令名/帮助文案修改专用）。

### 4.29 Issue #139 "删除既有命令 + 新增对称命令"双动作 Issue 标准模板

- **场景特征**：维护者本人发起，包含**两个独立动作**——① 删除既有命令（如 `/删群公告`）② 新增对称命令（如 `/取消群待办`，与 #131 `/添加群待办` 对称）。本仓库首次出现"双动作"Issue 模式，未来若再出现（如 `#解禁 ↔ #禁言`、`#取消头衔 ↔ #设头衔`）应直接套用。
- **核心纪律**：双动作工作量独立估算、分支独立判定、breaking-change 置信度按子诉求分别评估（**删除部分 0.90-0.95，新增部分 ≤0.30，不能统一给一个值**）。
- **应建议拆分 Issue**：维护者合并两条独立诉求时，分析必须显式建议拆分为两个独立 Issue 跟踪，除非有强约束必须合并实施——两条诉求的工作量、风险、决策点完全不同，混合 PR 会导致审查困难。
- **优先级决策路径（4 条必须显式）**：① 删除既有命令 + 跨适配器扩展 API → 最低 medium ✓ ② owner-driven + 已标准化模板（#131 沉淀） → 不升 high ✓ ③ 工作量可控（分支 B 5 天上限） + 迁移路径明确 → 不升 high ✓ ④ 跨适配器风险已识别但有降级路径 → 维持 medium。**应与 #131 群待办横向对比说明一致性**（同工作量、同跨适配器、同 owner-driven，定 medium 完全合理）。
- **必给标签基线**（breaking-change Issue 升级版）：`enhancement`(0.95) + `breaking-change`(0.85-0.95, 仅删除部分) + `deprecation`(0.75-0.85) + `command`(0.95) + `group-management`(0.85-0.92) + `onebot`(0.55) + `compatibility`/`onebot-extension`(0.75-0.85, 新增部分) + `permission-model`/`bot-role`(0.70-0.80, 涉及权限决策时) + `quote-message`/`reply`(0.70-0.85, 引用消息触发型) + `write-permission`(0.80, 撤销类写动作) + `needs-discussion`(≥0.80, 双动作决策点 ≥5 个) + `needs-info`(≤0.30) + `documentation`(0.40-0.75, 按同步工作量)。**应避免** `recall`(0.30, 与本 Issue 无关)。
- **可行性分支 A/B/C 必备 + 可选 D**：A（仅减法，~30 行 / 0.5-1 天）/ B（仅加法对称命令，复用 #131 模板，100-180 行 / 2-3.5 天）/ C（合并实施 250-400 行 / 3-5 天）/ **D（分阶段：A 先落地后 B 的 TODO，避免一次性大 PR）**。分支 A 纯减法也需七处同步（main.py + `_GM_COMMAND_NAMES` + README + 帮助 + CHANGELOG + 迁移指南 + 测试验证），不能省略。
- **分支 B 必给关键决策点**：① reply_id 来源（引用消息 vs 最近一条）→ 推荐引用消息 ② API 不支持适配器兜底（go-cqhttp 需 HTTP 插件）→ "禁用并提示" vs "直接抛错"决策树 ③ 是否需 `group_todo_disable_enabled` 配置开关（默认关闭 vs 开启） ④ 权限语义不对称（添加可能宽于撤销，QQ 群待办撤销通常需群主专属）。
- **写类对称命令权限不对称风险（新增）**：添加群待办可能允许群管，撤销群待办通常需群主。分析时必须显式对比"添加侧权限 vs 撤销侧权限"，与 §4.27 沉淀的"误解禁 vs 误禁言风险对比"对等。建议复用 `_is_group_owner` 而非 `_is_group_admin_or_owner`。
- **行号引用可溯源纪律**：精确行号（L2414-2433 等）若无"已读取 main.py 验证"说明，**必须**在可行性章节开头声明："以下行号基于项目记忆与既有 PR 模式推断，建议 PR 提交前以最新 main.py 行号为准"。
- **跨适配器 API 矩阵**（群待办撤销，需沉淀）：NapCat `_delete_group_todo(group_id, message_id)` / Lagrange `set_group_todo(group_id, message_id)`（参数同 #131）/ go-cqhttp 无标准群待办撤销 API（需 HTTP API 插件扩展）。
- **"API 返回 ok ≠ UI 生效"专项警示**（撤销场景更突出）：API 返回成功 → QQ 服务端写入 → 客户端 UI 异步刷新（1-2 秒）。撤销场景应提示"已撤销群待办，请刷新消息列表查看（约 1-2 秒后生效）"。建议撤销后调用 `get_group_todo_list` 回读校验。
- **撤回原消息场景**（必须列入必查项）：原消息被撤回后，群待办撤销时 reply_id 已失效，需明确兜底语义（"该消息已撤回，无法撤销对应群待办"）。
- **重复检测反模式持续触发**（#139 第 7 次命中禁用措辞清单）：本次"可能是 #138 的重复"命中禁用措辞；**应固化硬约束**：未来重复检测章节禁用措辞清单已扩展至包含"可能是#X的重复"。涉及同源但动作不同的 Issue（如 #138 删除 + #139 新增）应改用"与 #X 同源/相关但非重复，建议合并实施或分阶段"。
- **与 #131 显式互引**：分支 B 完全复用 #131 模板，应显式写"参 §4.21 OneBot 群待办类 Issue 分析模板（#131 案例）"以承接沉淀。重复检测章节显式写"非重复但与 #131 形成 add/cancel 对称命令，建议合并实施或分阶段"。
- **建议标题**：`[enhancement][medium] 移除 /删群公告 命令 + 新增 /取消群待办 命令（与 #131 对称）`（显式互引同类 Issue 便于追溯）；或更精简 `[enhancement][medium] 群待办撤销对称命令：移除 /删群公告 + 新增 /取消群待办`。
...
- **重复检测措辞模板**：无历史列表时**必须**写"暂未发现（建议检索关键词：...）"，不得写"未检测到重复"/"无重复"/"可能是 #X 的重复"。方向相反但同主题（如 #124 增强 vs #126 删除）严格归为 `related` 而非重复。**#138/#139 反思持续触发该红旗**：禁用措辞清单已扩展至"可能是#X的重复"，反思 checklist 须前置扫描禁用措辞。同源但动作不同的 Issue（如 #138 删 `/删群公告` + #139 增 `/取消群待办`）应改用"与 #X 同源/相关但非重复，建议合并实施"。
...
- **OneBot 群待办类命令七处同步清单**（新增 owner-driven 命令模板，参 §4.21）：main.py + README + 帮助命令 + `_conf_schema.json` + CHANGELOG + `_GM_COMMAND_NAMES` 元组注册 + 跨适配器兼容性文档。**删除既有命令反向清单**（参 §4.28 #138）：精确删除路径 + 保留路径边界 + 调用点清空 + README 帮助文案清理 + yield 帮助文本同步 + 配置 schema 语义缩窄 + bot 回复模板措辞清理 + CHANGELOG breaking-change 标注 + 替代方案提示。
