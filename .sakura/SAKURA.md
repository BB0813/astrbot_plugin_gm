...
### 4.28 Issue #138 命令别名/重命名决策类模板（owner-driven 减法+别名化）

- **场景特征**：维护者本人发起"删除既有命令 + 新增别名/重命名"复合诉求。本 Issue 典型为 `删除 /删群公告` + `新增 /取消群精华（/取消设精 别名化）`。属 owner-driven 减法+改名混合诉求。
- **关键洞察（避免重复实现）**：分析前必须先识别"等价命令"——`/取消群精华` 与 `/取消设精` 已存在完全等价命令，新增工作量为别名化而非新功能。
- **分类与优先级**：分类 `enhancement`；优先级 `medium`（或可考虑 `low`：主动减法 + owner-driven + 工作量小 + 无横切关注点，但因删除既有命令属 breaking-change 维持 medium）。**应在优先级章节显式列出 4 条决策路径**：① 主动减法+owner-driven → 应降 low ② 但涉及既有命令删除（breaking-change）→ 维持 medium ③ 别名决策未确定 → `needs-discussion` 高权重 ④ 跨适配器风险低（v11 标准）→ 维持 medium。
- **必给标签**：`enhancement`(0.95) + `command`(0.95) + `rename`/`alias`(0.75-0.85，建议仓库新建) + `breaking-change`(0.65-0.70) + `group-management`(0.85) + `needs-discussion`(0.75-0.85) + `deprecation`(0.70) + `documentation`(0.55-0.65)。`needs-info` ≤0.30（owner-driven 缺决策非信息）。
- **应避免的误标标签**：`parser`(0.45) 仅在涉及 `_extract_at_qq`/`_get_reply_id`/装饰字符 QQ 等解析逻辑改动时使用；`recall`(0.15) 仅在涉及 `recall_cmd`/`delete_msg`/2 分钟撤回时限等消息撤回场景使用——两者在本 Issue 均不涉及，应删除。
- **`/删群公告` 长期不工作的根因**：`delete_group_notice` 需 `notice_id` 参数，多数 OneBot 适配器对"按 ID 删公告"支持参差。替代路径：`send_group_notice_cmd` 中已有 `send_group_notice` 兜底。这是技术债清理而非破坏。
- **必查项**：① 既有等价命令识别 ② 双名共存迁移策略 ③ 引用消息触发适配 ④ 五处同步（main.py + 元组 + README + 帮助 + CHANGELOG）⑤ 别名决策（保留/共存/重命名）显式化。
- **可行性分支**：分支 A（最小：仅删除 `/删群公告`）~20-30 行 / 0.5-1 天；分支 B（完整：删除+新增别名）~120-150 行 / 1.5-2.5 天（边界测试单列半天）。
- **建议标题**：`[enhancement][medium] 移除 /删群公告 命令并新增 /取消群精华（与 /取消设精 共存，待确认）`。原标题清晰时标"可保留"。
- **重复检测措辞**：**禁止**"未检测到重复"——必须改写为"暂未发现重复 Issue，建议检索关键词：`/删群公告`、`/取消精华`、`notice_id`、`群精华取消`"。
- **"功能等价但命名不同"陷阱**：中文 bot 插件常见。未来分析应优先 grep `_GM_COMMAND_NAMES` 与所有 `@filter.command(...)` 装饰器，建立"命令名 → handler"映射，避免重复实现。

### 4.29 Issue #139 对称命令扩展类（"删除既有命令 + 新增对称命令"复合诉求）

- **场景特征**：维护者本人发起**两条独立诉求合并**：① 删除 `/删群公告`（裁剪）② 新增 `/取消群待办`（与 #131 `/添加群待办` 对称命令）。属 owner-driven 复合诉求。
- **必强制拆分建议**：分析必须显式建议拆分为两个独立 Issue 跟踪——两条诉求的工作量、风险、决策点完全不同，混合 PR 会导致审查困难。
- **breaking-change 置信度按子诉求分别评估**：删除部分 0.95，新增部分 ≤0.30（纯新增不构成 breaking），不能给统一置信度。
- **分类与优先级**：分类 `enhancement` + `medium`。**优先级 4 条决策路径必须全部显式列出**：① 删除既有命令 + 跨适配器扩展 API → 最低 medium ② owner-driven + 已标准化模板（#131 沉淀）→ 不升 high ③ 工作量可控（分支 B 5 天上限）+ 迁移路径明确 → 不升 high ④ 跨适配器风险已识别但有降级路径 → 维持 medium。
- **必给标签**（含 #138 沉淀的 `rename`/`alias`/`deprecation`）：`enhancement`(0.95) + `command`(0.95) + `group-management`(0.85-0.92) + `onebot`(0.55-0.70) + `compatibility`/`onebot-extension`(0.75-0.85) + `permission-model`(0.70) + `configuration`(0.65-0.75) + `needs-discussion`(0.75-0.85) + `breaking-change`(0.85 仅删除部分) + `deprecation`(≥0.85) + `quote-message`/`reply`(0.85) + `rename`/`alias`(0.75-0.85)。**禁止** `recall`(本 Issue 与撤回无关，应降至 0 或删除)。
- **写类对称命令权限不对称风险**（#139 新沉淀）：添加群待办可能允许群管，撤销群待办通常需群主（与项目其他命令"群管即可"权限模型不一致）。分析时必须显式对比"添加侧权限 vs 撤销侧权限"，与 #136 沉淀的"误解禁 vs 误禁言风险对比"对等。
- **七处同步清单对"删除既有命令"特别适用**：删除操作扩散点比新增更多（README/帮助/CHANGELOG/metadata/schema/元组注册/跨适配器文档），建议在可行性章节强制列七处而不是五处。
- **UI 异步刷新兜底**（#131 沉淀）：QQ 群待办 API 返回成功 → 服务端写入 → 客户端 UI 异步刷新（1-2 秒），撤销场景同样适用。建议指令提示"已取消群待办，请刷新查看（约 1-2 秒后生效）"。
- **撤回原消息场景**：删除引用消息时是否同时撤销群待办——必须在"必查项"中显式列入（#131 已识别，本 Issue 需复核）。
- **必查项**：① OneBot 适配器 `delete_group_todo` 支持矩阵 ② 撤销 API 权限要求 ③ 撤回原消息是否连带撤销待办 ④ UI 异步刷新兜底 ⑤ 权限不对称风险 ⑥ 重复检测措辞硬约束。
- **重复检测措辞**：**禁止**"未检测到重复"——必须改写为"暂未发现重复 Issue，建议检索关键词：`群待办`、`set_group_todo`、`群公告撤销`、`删群公告`"。
- **建议标题**：`[enhancement][medium] 删除 /删群公告 命令 + 新增 /取消群待办 命令（与 #131 对称，建议拆分 Issue）`。

### 4.30 "对称命令扩展"类 Issue 通用沉淀（#138 + #139 共建）

- **触发场景**：add/remove、enable/disable、set/unset、start/stop、bind/unbind 等对称命令；引用消息触发型命令的撤销变体；裁剪+新增合并诉求。
- **必查项（对称命令扩展标准 6 项）**：① 对称性（参数、权限、回执是否对称）② 共享 helper 抽取机会 ③ 一方已实现时另一方的依赖反转 ④ 权限语义可能不同（添加通常宽于撤销）⑤ 跨适配器 API 矩阵 ⑥ 撤销场景下原消息处理。
- **标签基线**：`enhancement`(0.95) + `command`(0.95) + `group-management`(0.85-0.92) + `onebot`(0.55-0.70) + `compatibility`/`onebot-extension`(0.75-0.85) + `permission-model`(0.70) + `configuration`(0.65-0.75) + `needs-discussion`(0.75-0.85) + `breaking-change`(0.85+ 若删除方) + `deprecation`(≥0.85) + `quote-message`/`reply`(0.85)。
- **"裁剪+别名化"混合诉求必须显式分支判定**：保留/共存/重命名三选一，不可默认走"新建命令"路径。
- **"功能等价但命名不同"是中文命令插件高频陷阱**（§4.28 共建）：`/取消精华` / `/取消群精华` / `/取消设精` 等多个名字共存是常见现象，未来分析应优先 grep `_GM_COMMAND_NAMES` 与所有 `@filter.command(...)` 装饰器建立"命令名 → handler"映射，避免重复实现。
- **复合诉求 Issue 强制拆分建议**（§4.29 #139 沉淀）：维护者合并两条独立诉求时，分析必须显式建议拆分为两个独立 Issue 跟踪，除非有强约束必须合并实施。两条诉求的工作量、风险、决策点完全不同，混合 PR 会导致审查困难。
- **同根因不同命令场景重复检测置信度上限**（§4.27 #136 共建）：主分类 + API 都相同时 0.75-0.85；症状/触发命令差异明显时上限 0.75；严格禁止"可能是#X的重复"措辞，改为"高度疑似重复（与 #X 同根因：...），建议合并修复"。

## 5. Issue 分析与标签经验（高层规则，详见 memory.md）

- 标题为"。"、"，"或信息极少时必须基于正文错误文本、复现命令和代码线索检索。
- **结构化输出校验失败 ≠ 信息不足**：应**修复字段输出**保留实质判断（分类/可行性/标签/关键问题列表），不得整体退化为"无法评估"——这是仓库反复出现的反模式。pre-check：任何字段出现"无建议/无法评估/空"前，确认是否源于校验失败。
- 不能因校验失败把 `bug`/`enhancement` 降级为 `other`、标签留空、可行性"无法评估"。
- 标签建议覆盖主类型、模块和风险；至少保留主标签与核心模块；高频模块标签：`recall`、`message-history`、`command`、`parser`、`onebot`、`group-management`、`moderation`、`stt`/`voice`、`title`/`special-title`、`mute`/`ban`/`ban-list`/`unmute`、`read-permission`/`viewer-role`、`onebot-extension`、`rename`/`alias`、`quote-message`/`reply`、`permission-model`、`bot-role`/`sender-role`、`decorative-unicode`/`unicode-normalization`。
- **重复检测措辞模板**：无历史列表时**必须**写"暂未发现（建议检索关键词：...）"，不得写"未检测到重复"/"无重复"/"可能是 #X 的重复"。方向相反但同主题严格归为 `related` 而非重复。
- **同根因不同症状 issue 重复检测置信度上限**：主分类 + API 都相同时置信度上限 0.75-0.85；症状/触发命令差异明显时上限 0.75。
- **撤回命令族五处同步清单**（高频踩坑）：`main.py` + `README.md` + 帮助文本 + `_conf_schema.json` + CHANGELOG；扩展为七处（多媒体转写层）：+ STT 配置字段 + 误伤率文档说明。删除既有命令场景强制列七处（§4.29 沉淀）。
- **群管理动作新增可细分为读类 vs 写类子模板**：写类关注写权限、API 返回、UI 生效延迟、回执；读类关注读权限、字段差异、数据脱敏、缓存策略、空状态/时间格式、侦察工具风险。
- **owner-driven + 涉及设计决策的 needs-discussion 校准**：即使维护者本人发起，只要涉及字段差异/权限/性能等设计决策，`needs-discussion` 应保持 0.65-0.85 高权重。
- **对称命令扩展类强制结构**（§4.30 #138 + #139 沉淀）：① 对称性（参数、权限、回执）② 共享 helper 抽取机会 ③ 一方已实现时另一方的依赖反转 ④ 权限语义可能不同（添加通常宽于撤销）⑤ 跨适配器 API 矩阵 ⑥ 撤销场景下原消息处理。
- **删除既有命令用法类 Issue 强制结构**（维护者自提减法）：① 精确删除目标 ② 保留目标边界 ③ 删除后旧语法处理 ④ 依赖解耦 ⑤ 用户迁移成本与告知 ⑥ CHANGELOG breaking-change 标注。
...
- **校验失败短路器（强制 Pre-check）**：校验失败应**局部修复字段输出格式**，不得把所有判断都退化为"无法评估"/`other`/空标签/无建议。实质性判断维持原始判断。
- **重复触发反模式的纪律性**：同一反模式反复触发说明反思流程缺乏自动化保护机制，必须固化 Pre-check 短路器到流程而非仅依赖记忆。
- **正确输出保底模板**（校验失败时仍须输出）：分类 + 优先级 + 摘要 + 建议标签列表 + 重复检测（"暂未发现"+ 关键词 + ⚠️措辞核对）+ 标题改写（"可保留"或轻量规范化）。
...
- **装饰字符 QQ 解析类 Issue 必给标签基线**（§4.25/§4.27 沉淀）：`bug` (0.95) + `command` (0.90) + `parser` (0.85) + `at-parse`/`at-extract` (0.80-0.85，建议新建) + `group-management` (0.75-0.80) + `unmute`/`mute-action` (0.85) + `mute` (0.70) + `decorative-unicode`/`unicode-normalization` (建议新建)。`onebot`/`compatibility` 0.35-0.50（根因在插件层而非协议层）。
- **装饰字符 QQ NFKC vs NFKD 关键技术陷阱**（§4.27 沉淀）：NFKC 对花体字 𝓒𝓪𝓷𝓬𝓮𝓻 / 数学字母 𝐀𝕒 / U+1D400-U+1D7FF Mathematical Alphanumeric Symbols **无效**——只能处理全角数字 ０-９ 这类兼容性分解字符。正确方案：① 白名单 `\d{5,12}` 强校验 + ② NFKD + 自定义映射表 或 ③ 直接拒绝非纯数字输入并提示。
- **装饰字符 QQ 传染性测试成本**（§4.27 沉淀）：13+ 调用点扫描 + 适配器差异测试 + 边界场景构造，传染性测试成本 +1 天而非 +0.5 天；总工作量 2-3 天。
- **命令别名/重命名决策类必给标签基线**（§4.28 #138 新沉淀）：`enhancement`(0.95) + `command`(0.95) + `rename`/`alias`(0.75-0.85，新建) + `breaking-change`(0.65-0.70) + `group-management`(0.85) + `needs-discussion`(0.75-0.85) + `deprecation`(0.70) + `documentation`(0.55-0.65)。`needs-info` ≤0.30。**禁止** `parser`、`recall`。
- **对称命令扩展类必给标签基线**（§4.29/#4.30 #139 新沉淀）：`enhancement`(0.95) + `command`(0.95) + `group-management`(0.85-0.92) + `onebot`(0.55-0.70) + `compatibility`/`onebot-extension`(0.75-0.85) + `permission-model`(0.70) + `configuration`(0.65-0.75) + `needs-discussion`(0.75-0.85) + `breaking-change`(0.85 仅删除部分) + `deprecation`(≥0.85) + `quote-message`/`reply`(0.85)。**复合诉求必须显式拆分建议**，`breaking-change` 按子诉求分别评估。
- **对称命令权限不对称风险**（§4.29/#4.30 沉淀）：添加群待办可能允许群管，撤销群待办通常需群主。分析时必须显式对比"添加侧权限 vs 撤销侧权限"，与 §4.27 的"误解禁 vs 误禁言风险对比"对等。
- **七处同步清单对"删除既有命令"特别适用**（§4.29 沉淀）：删除操作扩散点比新增更多（README/帮助/CHANGELOG/metadata/schema/元组注册/跨适配器文档），可行性章节强制列七处而不是五处。
- **UI 异步刷新兜底**（§4.21/#4.29 沉淀）：QQ 群待办类 API 返回成功 → 服务端写入 → 客户端 UI 异步刷新（1-2 秒）。撤销场景同样适用，必须显式提示用户"约 1-2 秒后生效"。
- **"功能等价但命名不同"中文命令陷阱**（§4.28 沉淀）：未来分析应优先 grep `_GM_COMMAND_NAMES` 与所有 `@filter.command(...)` 装饰器，建立"命令名 → handler"映射，避免重复实现。
