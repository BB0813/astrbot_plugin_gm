...
### 4.27 Issue #136 `/解禁` 装饰字符 QQ 解析 bug（同根因模式追加）

- **同根因模式**：与 #134（装饰字符 `@𝓒𝓪𝓷𝓬𝓮𝓻` 误识别）同源（`_extract_at_qq` 未对装饰字符做 Unicode 归一化与强校验）。`/解禁` 是 #134 修复路径的延伸触发点。
- **与 #134 横向对比**（同类 Issue 优先级对比矩阵）：#133（API/适配器 `duration=0` 语义）vs #134（禁言装饰字符 QQ）vs #136（解禁装饰字符 QQ）三者均为 `medium`，原因：① 单群单次未批量误操作；② 命令入口可快速防御；③ 工作量下限 1 天可交付。
- **误解禁 vs 误禁言风险对比**：`/禁言` 误识别装饰字符通常是无副作用空操作（API 失败）；`/解禁` 误识别装饰字符**可能误解禁正常用户**（如 `[\u202d@𝓒𝓪𝓷𝓬𝓮𝓻\u202d @123456789]` 中装饰字符未被拦截、取首机制错误指向 @123456789）。但若 API 对不存在目标返回 ok，则实际后果仍低，故维持 `medium` 不升 `high`。
- **必给标签（与 #134 模板严格对齐）**：`bug`(0.95) + `command`(0.92) + `parser`(0.85) + **`at-parse`/`at-extract`(0.85, 仓库可新建)** + **`unmute`/`lift-ban`/`mute-action`(0.85)** + `group-management`(0.80) + `onebot`/`compatibility`(≤0.35，可下调，非协议层) + `needs-info`(≤0.30，owner-driven + 修复方案已明确)。**反模式**：本次反思遗漏 `at-parse` + `unmute` 两个核心标签，必须对照模板逐项勾选。
- **关键技术细节（NFKC vs NFKD）**：装饰字符（Mathematical Alphanumeric Symbols U+1D400-U+1D7FF、Enclosed Alphanumerics U+2460-U+24FF）**NFKC 无法归一化**，只能处理兼容性分解字符（如全角数字 ０-９ → 0-9）。正确方案：① **白名单硬校验** `\d{5,12}`；② NFKD + 自定义映射表；或 ③ 直接拒绝非纯数字输入并提示"目标 QQ 格式不正确"。下次涉及装饰字符 QQ 修复必须明示这一点，不得笼统说"加 NFKC"。
- **多目标命令兼容性边界**：未明确 `/解禁 @A @B @C` 是否支持多用户——若支持，"取第一个合法 @ 目标"会破坏多用户场景；若不支持，应文档化并彻底拒绝多 @ 输入。**建议沉淀到项目记忆**：仓库"按目标用户操作"类命令的解析策略需明确多 @ 语义。
- **同类命令传染性**：`/禁言` `/解禁` `/踢人` `/设管` `/取管` `/头衔` 等命令均调用 `_extract_at_qq`，修复必须一次性扫描传染；测试矩阵 NapCat/Lagrange/go-cqhttp × 数学字母/手写体/Emoji 风格至少 3×4=12 个组合，建议统一为"小修复工作量下限 1.5 天（含测试矩阵）"。
- **owner-driven + 模板已沉淀的 `needs-info` 校准**：项目记忆 §#131 案例明确 owner-driven issue `needs-info` ≤0.30。本次标准模板命中、修复路径已明确，`needs-info` 应从常见 0.55 下调至 ≤0.30。
- **重复检测措辞红线**：`"可能是 #X 的重复"` 为禁用措辞；正确输出应写"与 #134 同根因模式（`_extract_at_qq` 解析），建议合并修复；非 #128/#125/#133 的重复"。**`needs-info` 与 `onebot`/`compatibility` 校准常被忽视**，本次反思暴露了双重校准失败（模板对齐失败 + 措辞红线触发）。
...
- **装饰字符 QQ 解析模板必给标签**（§4.25-§4.27）：`bug` + `command` + `parser` + `at-parse`/`at-extract`（仓库可新建） + `group-management` + `mute`/`unmute`/`mute-action` + `needs-info` ≤0.30 + `onebot`/`compatibility` ≤0.35；分析时**对照模板逐项勾选**不允许遗漏。
- **Unicode 装饰字符归一化陷阱**（§4.27）：Mathematical Alphanumeric Symbols (U+1D400-U+1D7FF)、Enclosed Alphanumerics (U+2460-U+24FF) NFKC **无法**归一化为 ASCII；修复必须用 ① `\d{5,12}` 白名单硬校验 ② NFKD + 自定义映射表 ③ 或直接拒绝非纯数字输入并提示"目标 QQ 格式不正确"。不得笼统说"加 NFKC"。
- **同类 Issue 优先级对比矩阵**（§4.27）：所有"装饰字符 QQ 解析"模式 Issue（#133/#134/#136）应建立 Issue 号 / 触发命令 / 副作用方向（误禁言 vs 误解禁 vs 误踢人） / 优先级 / 备注 五列表，让读者一眼看清为何同模式有差异。
- **跨适配器读取字段差异矩阵**（§4.26）：`role` / `shut_up_timestamp`（NapCat/go-cqhttp） / `ban_expire_time`（Lagrange） / `mute_end_time`（部分实现） / `last_sent_time` / `join_time` / `level` / `special_title` 的跨实现差异对照表，未来涉及成员列表过滤、权限校验、状态查询的 Issue 必须主动列出 fallback 表。
- **群管理动作新增模板分化为读类/写类**（§4.26）：写类（#131 群待办）关注写权限/UI 生效延迟/回执；读类（#135 禁言列表）关注读权限/字段差异/数据脱敏/缓存策略/侦察工具风险。
- **小修复工作量下限校准**（§4.25-§4.27）：装饰字符 QQ 类小修复（<50 行）工作量下限应为 **1.5 天**（含跨适配器测试矩阵 NapCat/Lagrange/go-cqhttp × 4 类装饰字符 = 至少半天到 1 天），不得低估为 1 天内。
- **重复检测措辞红线（固化）**（§4.27）：禁词清单 `"未检测到重复"` / `"无重复"` / `"可能是 #X 的重复"` / `"无法评估"` / `"无建议"`；必须写"暂未发现"+ 建议检索关键词 + `⚠️ 措辞核对` 末注。每次反思 Pre-check 必查项。
