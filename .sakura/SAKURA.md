...
- **增量审查风险点**：`chore: reflection` 类提交可能修改逻辑，增量审查应获取完整提交链验证反思是否已体现

### 3.16 PR 描述完整性校验（PR #156 强化）
...
**Quick策略"声明式变更"检查（PR #157 新增）**：
- 变更文件清单获取：git diff --name-only（实际变更）+ PR 描述提到的文件（声明变更），取并集作为审查范围边界
- 规则S升级：PR描述提到的文件与实际变更文件一致，差异原因需明确标注（代码变更遗漏/文档未同步/PR描述过时）
- 增量审查涉及状态机/计数器时，应扩展至完整方法上下文
...
**配置迁移的隐形风险**：从 `get_group_setting` 改为 `self.config.get` 后，群组配置中残留的旧值既不生效也不会被清除
...
- **⚠️ Quick策略"声明式变更"检查**：变更文件清单需取 git diff 与 PR 描述的并集；差异原因需明确标注
