...
### 4.24 禁言列表查询命令 Issue #135 沉淀（owner-driven 读类命令）

- 维护者自提新增 `/禁言列表` 命令，调用 `get_group_member_list` 过滤 `shut_up_timestamp`/`ban_expire_time` 字段展示本群被禁言成员。
- 分类 `enhancement` + `medium`：与 #131 群待办同级但工作量更小、跨适配器风险更低（不依赖 set_group_todo 那类非标准 API）；优先级决策路径需在分析中**显式写出**（避免读者横向对比产生质疑）。
- **读权限 vs 写权限**是新标签分化方向：仓库现有 `permission` 笼统，建议细化为 `write-permission`/`read-permission`/`viewer-role`。
- **跨适配器字段差异矩阵**（读取类成员字段）：`shut_up_timestamp`（NapCat/go-cqhttp）vs `ban_expire_time`（Lagrange）vs `mute_end_time`（部分实现）。任何"群成员 X 字段"查询类 issue 都应主动列 fallback 表。
- **读权限可能被滥用为侦察工具**：查看禁言列表本身揭示群管执法记录，**需要维护者决策可见范围**——这是 `needs-discussion` 高权重的核心依据（应保持 0.65-0.75，不是 0.4）。
- **重复检测前置过滤**：与 #131 都是 `enhancement`+owner-driven+群管理命令，但 API 完全不同（`set_group_todo` vs `get_group_member_list`），不能机械按表面相似度标重复。措辞必须"暂未发现"+关键词+"⚠️ 措辞核对"。
- 建议新增 `mute`/`ban-list` 标签子体系（与 `title`/`recall`/`vote` 并列）。
- 工作量估算陷阱：分页/分批处理缺失 + 缓存策略 + 空状态提示 + 时间格式边界 + 隐私边界——看似 100-200 行实际可能更高。

### 4.25 校验失败整体降级反模式第三次集中爆发（Issue #132、#133 两次、#134）

- 本批反思**再次完整触发**"校验失败短路器"失效：#132 分类 `other`、可行性/标题"无法评估/无建议"、标签空、重复检测写"未检测到重复"——典型五连退化；#133（378d116 第一次）分类 `other`、可行性"无法评估"、标题"无建议"；#133（98310f2 第二次）分类 `bug` 正确但重复检测写"可能是 #128 的重复"；#134 分类 `other`、可行性"无法评估"、标题"无建议"、标签空、重复检测写"可能是 #125 的重复"。
- **Pre-check 短路器（强制规则）**：反思摘要出现 `expected <REASON>`/`expected <SUGGESTED_TITLE>` 等校验错误时，**只修复字段输出格式**，实质性判断（分类/优先级/标签/可行性/标题/重复检测）必须基于 Issue 实质内容独立完成，**不得连带退化**——这是仓库**第四次**重复触发的反模式（PR #123 第六轮 + #130 + #131 三轮 + #132 + #133 两次 + #134）。
- **必须固化的机制**（反思指令层）：
  1. 反思输出前自动扫描禁用措辞（`other` + `无法评估` + 空标签 + `无建议` + `未检测到重复` + `可能是 #X 的重复` + `未发现重复` 简写）；
  2. 任何字段出现禁用措辞时**撤销该字段单独降级**，保留原始判断；
  3. 优先级决策路径必须在分析中**显式列出四条**（最低优先级、不升 high 条件、维持 medium 条件、不升 high 但有兜底）；
  4. 重复检测章节必须含"暂未发现"+ 检索关键词 + "⚠️ 措辞核对"三段式；
  5. 标题改写禁止"无建议"——原标题清晰时直接给"可保留"或轻量规范化。

### 4.26 解禁类 Issue 标准分析模板（Issue #133 沉淀）

- **触发场景**：`/解禁 @用户` / `unmute_cmd` 提示成功但目标仍被禁言。
- **必查项 9 维**：① Bot 在该群角色（部分协议要求群主专属解禁）② 目标用户是否仍在群内（退群兜底）③ OneBot 实现版本（NapCat/Lagrange/go-cqhttp 对 `delete_group_ban` 语义差异）④ `duration` 参数语义（`duration=0` vs 不传 vs `duration=-1`）⑤ `_extract_at_qq` 解析（必须按 segment/user_id，不按空格 split）⑥ `get_group_member_info` 回读校验（API 返回 ok ≠ 实际生效）⑦ 早退语句吞噬共享逻辑（权限/参数校验早退导致回读/日志未执行）⑧ 完整执行日志 + 最近配置变更 ⑨ `falsy` 判空 vs 严格判空（`if not duration` 会把 `0` 误判为未传）。
- **必给标签**：`bug` + `command` + `group-management` + `unmute`/`lift-ban`（仓库新建建议）+ `onebot` + `compatibility` + `bot-role` + `needs-info` (≥0.85)。
- **优先级**：`medium`（核心命令局部不可用 + 误判用户状态风险 + 跨适配器兼容风险已识别但有兜底）。
- **禁言/解禁语义至少 3 层**：群管 API / 插件自怼 / 审批工作流待审批——解禁作为禁言的逆向动作同理也分 3 层，必须先确认用户指哪一层（避免误改另一层）。

### 4.27 装饰字符 QQ / 视觉欺骗型用户名新晋高频解析类 bug 模式（Issue #134 沉淀）

- **触发关键词**：`@用户` + `解禁`/`禁言`/`踢人`/`设管` + 装饰字符/数学字母/花体字（如 `𝓒𝓪𝓷𝓬𝓮𝓻`、`𝐀`、`𝕒` 等数学字母块 Unicode）。
- **根因**：`_extract_at_qq` 未做 NFKC 归一化 + 数字段强校验，装饰字符 QQ 经 AstrBot 事件框架后保留为字符串，被命令解析误识别为合法目标。
- **必查项**：① `_extract_at_qq` 是否 NFKC 归一化 ② OneBot 适配器是否对装饰字符 QQ 拒绝/截断 ③ 群号是否也污染 ④ 是否需在输入层加"QQ 必须是纯数字"硬校验 ⑤ 装饰字符 QQ 跨适配器命令成功率矩阵。
- **修复方向**：`_extract_at_qq` 顶部加 `unicodedata.normalize('NFKC', s)` + 正则 `\d{5,12}` 强校验 + 错误提示"目标 QQ 格式不正确"。
- **重复检测陷阱**："症状相似 ≠ 根因相似"——#125（头衔）与本 Issue（禁言）都用"获取信息问题"作为症状描述，但根因模块不同（`special_title` vs `_extract_at_qq`）。下次遇到"获取信息问题"+"命令解析错误"模板时**先按模块归类再判断重复**，不要按症状字符串相似度。
- **项目记忆激活范围**：模板触发关键词应扩展覆盖"解禁/禁言"，而非仅"头衔类"。
- **优先级应纳入"误解禁安全影响"**：误解禁放大已有禁言执行错误，安全权重 > 误禁言——优先级评估应纳入此权重（`medium` 维持，不升 high 但也不降 low）。
- 工作量估算 20-60 行（条件判断+测试用例），但**测试矩阵构造**（NapCat/Lagrange/go-cqhttp × 数学字母/手写体/Emoji 风格）至少 0.5 天，小修复下限 1 天而非 1 天内。

### 4.28 权限模型重构 Issue #132 重复触发 #130 教训

- 本 Issue 与 #130 几乎完全相同的"权限模型重构"模式（同一维护者 + 标题关键词 + 模式高度一致），应主动建议合并或互相引用，避免分散精力。
- 完整正确判断（参 §4.17、§4.20 模板）：分类 `enhancement` + 优先级 `medium` + 标签 `enhancement` + `breaking-change`(≥0.90) + `deprecation`(≥0.75) + `permission-model`(≥0.80，新建) + `bot-role`/`sender-role`(≥0.70，新建) + `configuration` + `group-management` + `onebot` + `needs-discussion`(0.75)；可行性 100-200 行（移除配置读取 + 改写权限 helper + 迁移默认行为）+ 横切影响所有 `plugin_admins`/`has_*_admin_rights` 调用点 + 权限提升审计 + 配置迁移路径 + API 失败兜底。
- **重复检测须明示关联**：与 #130 关系应为**细化实施方案**（重复/延续/related），不是"未检测到重复"。
- **维护者连续提两个权限重构 Issue 的信号**：正在主动重构权限层，必须主动建议互相引用/合并。
