...
- 重复检测不能只依赖标题，**措辞要稳**：无历史列表时**写"暂未发现"并列出建议检索关键词**，"未检测到重复"/"无重复"/"可能是 #1 的重复"均被明令禁止。同命令/同模块但症状、触发路径、修复点不同的 Issue 应标关联而非重复。**重复判断要比较完整用户故事链**（私聊申请→管理员审批→自动解禁→通知），私聊解禁 vs 群内解禁应标"关联"而非"重复"。
- 对信息不足的 Issue，仍应给有条件的可行性判断：列出需要补充的证据，以及常见代码结构下的预估工作量与风险。
- 对"版本已更新仍存在"的问题，应追踪最近相关 commit/PR：可能是修复方向错误、覆盖入口不全、插件目录/缓存未更新。

### 8. 本次新增经验（#125、#126、PR#123 反思提炼）

- **"删除既有命令用法"类 Issue 六要素结构**：① 删除目标（精确到 Path 分支/行号）② 保留目标（哪些路径不受影响）③ 边界行为（删除后旧语法如何报错/回退）④ 依赖解耦（被删功能下游是否需一并调整，如 `/消息列表` ↔ `/撤回 编号`）⑤ 用户迁移（迁移成本与告知方式）⑥ CHANGELOG（是否标注 breaking change）。
- **撤回类 Issue 标签三件套**：`recall` + `command` + `parser` 必须齐全，不容遗漏。
- **`help wanted` 对 owner-driven self-implementation 不适用**：维护者本人提交 + 改法明确 + 工作量小 → 置信度 ≤0.05 或不推荐。
- **行号定位必须标注证据来源**：精确行号（如 L2024-2057）若无"已读取 main.py 验证"说明，应改为"约 L2000-2060"等模糊表述，避免猜测性精确化。
- **PR 描述数字与 diff 不一致必须主动指出**（如 PR 描述 +520/-103 实际 +494/-52），不放过。
- **"代码已尽力"的提示文案设计**：当失败发生在协议层（Bot 权限 ≤ 目标用户 QQ 硬限制）时，提示应包含已尝试策略 + 最可能环境原因 + 用户可执行下一步，而非仅技术性回执（如"仍被读到"）。README 也应显式说明 `/取消头衔` 的限制条件。
- **环境信息收集的代码化**：建议新增调试命令（如 `/头衔诊断 @用户`）输出 Bot 群角色、目标用户身份、当前 `special_title`、API retcode，将"需补充诊断信息"转化为自助排查工具。
- **标签 checklist 显式化**：每次分析应主动核对 ① 主分类 ② 模块标签（command/parser/group-management/recall/title/...）③ 适配器标签（onebot/napcat/lagrange/...）④ 决策标签（needs-info/needs-discussion/help wanted/...），避免遗漏。
- **增量审查的"对接面扫描"四步**：① 读取改动前代码确认既有接口契约 ② 走端到端流程 ③ 横切关注点"未变更则保留、变更则重审"清单化 ④ 抽样验证对称路径（如 `recall_cmd` ↔ `recall_user_cmd` 缓存回退对称性）。
- **`ast.parse` ≠ 真实加载**：不执行装饰器、不触发命令注册，不能作为加载验证替代品；至少 `python -m py_compile main.py` 或最小 AstrBot 插件加载验证。
- **撤回缓存三层链路优先级**：必须显式定义并审查 `recent_messages`（旧）→ `message_history`（新）→ `get_group_msg_history`（OneBot 兜底）三层链路的优先级与失败回退。