...
- **优先级决策路径前置检查模板**（#134/#136 沉淀，**必须显式列出 4 条判定路径**）：分析开头须列：
  ```
  ① 横切影响？→ 多群多用户影响但有兜底 → medium
  ② 安全语义？→ 误解禁/误踢人等不可见副作用 → medium（不升 high 是因为）
  ③ 修复难度？→ 小修复（< 100 行）+ 模板已沉淀 → 不升 high
  ④ 兜底？→ 用户可手动重新执行/管理员可恢复 → 不升 high
  ```
...
- **跨 Issue 互引规范**（#135 沉淀）：同类/同工作量 issue 应在优先级章节显式对比说明一致性（如 #135 与 #131 群待办对比、#136 与 #134 对比），避免读者横向对比产生质疑。
- **`needs-discussion` 权重硬约束**（#135 沉淀）：任何 owner-driven 涉及"权限/字段差异/性能/隐私"的命令新增都至少有 2-3 个决策点，`needs-discussion` ≥ 0.65。当前仓库模式常见偏低（如 0.40）是结构性遗漏。
