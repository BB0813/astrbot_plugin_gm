...
- **工作量**：20-60 行 + 1-2 天（条件判断 + 测试矩阵半天，保守下限 1 天）。
- **同症状不同模块陷阱**：与 #125（头衔类）症状字符串都含"获取信息问题"，但根因模块完全不同——重复检测必须**按模块归类**而非按症状相似度，避免误报 #125。
- **优先级决策路径**：① 单群单次 → 不升 high ② 核心命令行为错误 → 最低 medium ③ 误解禁比误禁言风险更高（用户处于禁言态时误解禁会放大后续违规）④ 但有补救路径（重新禁言）→ 维持 medium。
- **决策候选**：主分类绝不能是 `other`；`parser`/`at-parse`/`at-extract` 是核心标签；`needs-info` 缺日志/版本/适配器/装饰字符来源时 0.55+。
- **建议标题**：`[bug][medium] /解禁 @装饰字符用户 解析异常：装饰字符 QQ 被误识别为目标 QQ` 或 `[bug] /解禁 命令对装饰字符 @用户 的解析错误`。
...
- **标签基线（必给）**：`enhancement`(0.95) + `breaking-change`(≥0.90) + `deprecation`(≥0.75) + `permission-model`(≥0.80) + `configuration`(0.85) + `group-management`(0.85) + `bot-role`/`sender-role`(≥0.70) + `onebot`/`compatibility`(≥0.65) + `needs-discussion`(0.75-0.85) + `needs-info`(≤0.45)。`question` 必删。
...
- **可行性四段式骨架**（高复用）：① 已有可复用机制 ② 必改 N 项（精确行号+读取验证）③ 风险点（完整覆盖 API 失败兜底/缓存策略/跨适配器/配置迁移/权限提升审计/同类配置项）④ 验证清单。
- **决策候选**：`question` 标签在有明确诉求时**应删**（非询问类）。
...
- **重复检测措辞强制模板**：无历史列表时**写"暂未发现"并列出建议检索关键词**，"未检测到重复"/"无重复"/"可能是 #X 的重复"均被明令禁止。每次重复检测输出末加注"⚠️ 措辞核对：是否使用了禁用的'未检测到重复'/'无重复'/'可能是#X的重复'？"。**已知至少 4 次触发**（#131/#132/#133/#134）。
- **重复检测前置过滤**：若两 Issue 主分类不同（一个 bug 一个 enhancement），置信度上限 0.3，避免基于关键词表面相似度的误报（#132 曾误报 #125）。
- **决策候选约束**：遇到有明确诉求的 issue，`question` 标签**必删**（已纳入权限模板 v2 普适规则）。`other` 仅在确实无法归入现有体系时使用，禁止作为校验失败的兜底。
...
- **可行性必须显式分支判定**：分析中应给出"分支A（API 层兼容）~N 行/天 → 分支B（补状态回读链路）~M 行/天 → 分支C（重构入口或权限校验）~K 行/天"，不要只给单一范围估算（#133/#132 反复未显式分支）。
- **校验失败拦截规则（反思助手 Pre-check）**：若本次输出出现"无法评估/other/空标签/无建议/未检测到重复"且摘要提及字段校验错误（如 `expected <SUGGESTED_TITLE>`/`reserved tag syntax`），**自动撤销降级并按 Issue 实质内容恢复原始判断**——字段校验仅触发输出修复，不得影响分类/优先级/标签/可行性/标题/重复检测等实质性判断。**至少已触发 4 次**（#131/#132/#133/#134）。
