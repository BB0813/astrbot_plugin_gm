...
仓库 `mjy1113451/astrbot_plugin_gm` 是 AstrBot 群管理类插件，Issue/PR 常涉及命令解析、撤回、群管理权限、按群配置、OneBot 适配器兼容、配置 schema 与 README 同步。**高频模式**：权限模型重构（#130/#132）、引用消息触发型命令（#131 群待办）、撤回/STT/头衔/加群申请、**装饰字符 QQ 解析（#134）**、**解禁/解除禁言（#133）**、**禁言列表查询（#135）**——出现关键词即套对应模板。
...
- **跨适配器"读取类群成员字段"对照表需求**（#135 沉淀）：`role`、`shut_up_timestamp`（NapCat/go-cqhttp）、`ban_expire_time`（Lagrange）、`mute_end_time`（部分实现）、`last_sent_time`、`join_time`、`level`、`special_title` 的跨实现差异矩阵应在项目记忆中正式建立。

### #136（误解禁）专属沉淀

- **`/解禁` 装饰字符 QQ bug 优先级判定补充**：同根因模式 #134 已 medium，#136 同源但**可能误解禁本应继续禁言的正常用户**（若第一个 @ 恰好是另一个被禁言成员）。风险对比：误解禁"不可见"（用户不知已被解禁）vs 误禁言"可见"（立即收到通知），**误解禁检测延迟与修复成本更高**。但仍维持 medium，理由：① 需特定群内触发 ② 仅影响单次命令解析层 ③ 修复模板明确 ④ 用户可手动重禁。
- **`/解禁` 同类命令传染性测试**：修复分支 A/B 应同时扫描 `/禁言` `/踢人` `/设管` `/取管` 调用 `_extract_at_qq` 入口，传染性测试开销应计入分支 C 工作量。
- **#136 与 #134 重复检测**：输出必须"暂未发现 + 与 #134 同根因模式（_extract_at_qq 解析），建议合并修复；非 #128/#125/#133 重复"——禁用"可能是 #128 的重复"措辞。
- **#136 标签漏项**：本次反思漏掉 `at-parse`(0.85) + `unmute`(0.85)，`needs-info` 应从 0.55 下调至 ≤0.30（owner-driven + 模板已沉淀）。
- **#136 可行性分支 B 与 A 重叠**：分支 B 描述的"取第一个合法 @ 目标"与分支 A 同文件同函数，应整合；分支 C 工作量应计入同类命令传染性测试开销。

### 装饰字符 QQ 修复的 NFKC vs NFKD 技术细节（#134/#136 沉淀，**关键技术纠正**）

- **NFKC 不能解决花体字问题**：装饰字符（Mathematical Alphanumeric Symbols U+1D400-U+1D7FF、Enclosed Alphanumerics U+2460-U+24FF）的归一化**不适用于 NFKC**——NFKC 只能处理兼容性分解字符（全角数字 ０-９ → 0-9），**不会把花体字 𝓒𝓪𝓷𝓬𝓮𝓻 拆成 ASCII**。
- **正确方案三选一**：① **白名单方案**（推荐）：`_extract_at_qq` 只接受 `\d{5,12}` 纯数字，拒绝一切非纯数字输入并提示 ② **NFKD + 自定义映射表**（把常用装饰字符映射到 ASCII）③ **直接拒绝 + 提示**（最保守）。
- **下次分析涉及装饰字符 QQ 修复时必须明确这一点**，不能笼统说"加 NFKC 归一化"。
- **同源项目记忆 §装饰字符 QQ 模板的"修复建议"段落需同步更新**（`unicodedata.normalize('NFKC', s)` → 改为白名单/NFKD+映射/拒绝）。

### 装饰字符 QQ 类 Issue 横向对比矩阵（#133/#134/#136 沉淀）

| Issue | 触发命令 | 副作用方向 | 优先级 | 备注 |
|---|---|---|---|---|
| #134 | /解禁 /禁言 | 误禁言装饰字符 QQ（空操作，API 失败） | medium | 解析层 bug |
| #136 | /解禁 | **误解禁正常用户**（可能产生实际副作用） | medium | 比 #134 更敏感，但修复模板一致 |

- **误解禁 vs 误禁言风险对比**（#136 沉淀）：前者可能产生无意义空操作，后者可能误解禁一个本应继续禁言的用户。**误解禁是"不可见的"（用户不知道已被解禁），误禁言是"可见的"（用户立即收到通知）**，因此误解禁的检测延迟和修复成本都更高。
- **未升 high 的共同理由**：① 仍需特定群内触发 ② 仅影响该单次操作的命令解析层 ③ 修复方案明确（小范围白名单/NFKD+正则即可）④ 已有兜底（用户可手动重新禁言）⑤ 跨适配器测试矩阵已建立。
- **建议在优先级章节显式做横向对比表**，避免读者横向质疑为何同模式 Issue 优先级有差异。

### 多 @ 设计行为 vs bug 的边界（#134 沉淀）

- `_extract_at_qq` docstring 是"Extract first QQ number"——这是**设计行为但未在帮助/README 中显式说明**。
- `kick_cmd` 已用 `_extract_at_qqs` 实现多 @ 批量模式，意味着多 @ 是仓库"已经在用但未完全迁移"的设计。
- **建议在分析中明确**：`unmute_cmd`/`mute_cmd` 未迁移是**设计不一致 bug**（不是单纯的设计行为）。
- **未明确仓库是否支持多 @ 用户作为命令参数**（`/解禁 @A @B @C`）——影响所有"按目标用户操作"类命令的解析策略。若支持，"取第一个合法 @ 目标"会破坏多用户场景；若不支持，应明确文档并彻底拒绝多 `@` 输入。
- **传染性测试成本**（#136 沉淀）：同类命令扫描（`/禁言`/`/解禁`/`/踢人`/`/设管`/`/取管`）的传染性测试开销应计入分支 C 工作量。

### 装饰字符 QQ 项目记忆模板对齐检查（#134/#136 沉淀）

- **分析涉及装饰字符 QQ / @解析 / 禁言类 Issue 时，**第一步**必须对照项目记忆对应模板的"必给标签"/"必查项"清单逐项勾选**，不允许遗漏（如 `at-parse`、`unmute`、`mute-action`）。
- **若 Issue 编号已有专属标准分析模板**（如 #134 案例模板），实质性判断（分类/优先级/标签/修复方向）必须与模板对齐；仅在模板未覆盖的新信息出现时做增量分析，不得偏离模板核心结论。
- **根因定位偏差陷阱**：看到表面症状（多 @ 取首个）就下结论，而忽略项目记忆已有的核心根因模板（装饰字符 QQ）——本质与"看到表面症状就下结论"是同类问题。

### `_extract_at_qq` 函数族的系统性问题（#134 沉淀）

- `_extract_at_qq`（单 @）、`_extract_at_qqs`（多 @）、`_parse_qq_list`（数字 QQ 提取）三个函数都缺乏 NFKC/NFKD + 正则强校验。
- **建议未来重构时统一为单一解析入口**，避免分散实现导致修复遗漏。
- **修复涉及的 NFKD+正则校验应统一应用到所有调用点**，包括 #125（头衔）、#131（群待办）、#133（解禁）的相关路径——这是"系统性问题统一修复"信号。
...
### 装饰字符 QQ / 视觉欺骗型用户名 Issue 标准分析模板（#134 案例，**新晋高频解析类 bug 模式，已第四次触发校验失败短路**）
...
- **工作量**：20-60 行 + 1-2 天（条件判断+测试矩阵半天，保守下限 1 天，包含最小适配器验证）。
...
### 群管理动作模板 v2：写类 vs 读类细分（#135 新沉淀）

- **写类**（#131 群待办、禁言/解禁/头衔/踢人）：关注写权限、API 返回、UI 生效延迟、回执。
- **读类**（#135 禁言列表查询）：关注读权限、字段差异、数据脱敏、缓存策略、可见范围控制。
- **读类额外必查项**（#135）：① 大群分页/分批（`get_group_member_list` 在 500+ 人群是否分页）② 成员列表缓存策略与失效 ③ 空状态友好提示（"当前群无被禁言成员"）④ 时间格式显示边界（>30 天、永久）⑤ 隐私考虑（昵称+QQ号展示范围）⑥ 侦察工具风险（是否需限制可见范围）。
- **建议仓库新增 `mute`/`ban`/`ban-list` 标签子体系**：与 `title`、`recall`、`vote` 并列，禁言相关 issue 多发，便于检索。

### 读类群管理命令标签基线（#135 沉淀，**未来同类命令新增的标签模板**）
...
needs-discussion(0.65-0.75) + configuration(0.65-0.75) + 
needs-info(≤0.20) + help-wanted(≤0.10) + good-first-issue(≤0.10)
```

- **`needs-discussion` 权重提醒**（#135 沉淀）：任何 owner-driven 涉及"字段差异/权限/性能"的命令新增都至少有 2-3 个决策点，`needs-discussion` 应保持 ≥0.65。当前仓库模式常见偏低（如 0.40），是结构性遗漏。
- **标签子体系建议新增**（仓库标准化推动）：`mute`/`ban`/`ban-list`/`mute-status`/`read-permission`/`viewer-role`/`onebot-extension`/`bot-capability`。
- **`onebot-extension` 必要性**：`shut_up_timestamp`/`ban_expire_time` 等字段在 OneBot v11 中非标准字段，应明确标 `onebot-extension`（仓库若可新建），与 #131 群待办的非 v11 标准问题对等。
...
- **同类 issue 显式互引**：同类/同工作量 issue 应在优先级章节显式对比说明一致性（如 #135 与 #131 群待办对比），避免读者横向对比产生质疑。
- **读权限 vs 写权限区分**（#135 新沉淀）：本仓库 issue 多关注"谁能执行"（写权限），但"群管理列表查询"涉及"谁能查看"（读权限）。仓库标签体系若仅有 `permission`，应细化为 `write-permission` / `read-permission` / `viewer-role`。读权限可能被滥用为侦察工具，需评估可见范围。
- **重复检测前先做"主分类+API"双过滤**（#135 沉淀）：#131（群待办）与 #135（禁言列表）主分类同是 enhancement 但 API 完全不同（`set_group_todo` vs `get_group_member_list`），不应机械按表面相似度标重复，置信度上限 0.30。
- **行号引用溯源标准格式**（#134 沉淀）：精确行号须配套 commit SHA 或读取时间戳（如"基于 commit XXXX 的 main.py L1937-1954"），否则读者无法判断时效性。建议：要么读取验证并标注，要么用"约 L2000-2060"等模糊表述。
- **优先级决策路径模板前置检查**（#134 沉淀）：优先级章节开头必须列出 4 条判定路径——① 横切影响？② 安全语义？③ 修复难度？④ 兜底存在？让读者能追溯判定逻辑。
- **可行性分支关系必须显式说明**（#134 沉淀）：A、B、C 是合并（A+B=C）还是互斥？工作量是否可叠加？推荐顺序：先发哪个？为什么？影响代码审查和回滚成本。
- **标题改写前必须说明改写理由**（#134 沉淀）：原标题遗漏了什么核心信息？本次改写覆盖了哪些遗漏？这是避免维护者误读改写的必要注释。
- **`needs-info` 标签使用不严谨陷阱**（#134 沉淀）：当"未知"是"跨适配器测试矩阵"而非"修复决策所需信息"时，应使用 `needs-testing`（仓库若可新建）或 `compatibility` 而非 `needs-info`。装饰字符 QQ 是结构性问题，修复方案模板已沉淀，真正缺的是测试矩阵而非决策信息。
- **`security`/`safety`/`input-validation` 标签遗漏**（#134 沉淀）：装饰字符 QQ 误识别是"输入校验失败"的安全语义问题，应加入 `security`(0.50-0.65) 或 `input-validation`（仓库可新建）。
- **校验失败短路反模式升级为硬约束**（#135 沉淀）：#135 是该项目记忆反复警告的"校验失败→整体降级"反模式的典型案例（分类 `other`、可行性 `无法评估`、标题 `无建议`、标签空、重复检测 `未检测到重复`）。**反模式自检门槛前置**：反思输出落笔前必须扫描 6 条硬约束（字段校验失败仅局部修复 / 标题/标签/可行性/重复检测不可写禁用措辞 / 实质性判断不得连带退化）。
