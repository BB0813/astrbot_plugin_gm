...
- **必给标签**：`enhancement`(0.95) + `command`(0.85-0.95) + `configuration`(0.80-0.90) + `group-management`(0.85-0.92) + `permission-model`(0.80-0.85) + `kick`(0.85) + `message-history`(0.75-0.85, 新建) + `auto-action`(0.70-0.80, 新建) + `privacy`(0.70-0.80, 新建) + `onebot`(0.55-0.90) + `compatibility`(0.55-0.75) + `external-reference`(0.85, 参考而非合入) + `breaking-change`(0.40-0.65) + `needs-discussion`(0.70-0.85) + `needs-info`(≤0.30, 双校准对冲)。
...
- **优先级决策路径 5 条（#142 升级默认值决策）**：①②③④ 同前 + ⑤**默认值决策（默认 false → 维持；默认 true → 升 high/silent behavior change）**。
...
- **校验失败短路器（P0）**：字段校验失败**仅修复字段输出**，**不得整体降级为"无法评估"/`other`/空**。**Pre-check**：含"无建议/无法评估/空/未检测到重复"前必先确认是否源于校验失败——校验失败应**局部修复**。**实质性判断不得连带退化**。**#143 整体降级**。**"#143 恢复模式"**：下次遇整体降级，**先恢复实质性判断再修复字段输出**。
