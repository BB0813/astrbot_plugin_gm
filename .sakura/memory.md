...
仓库 `mjy1113451/astrbot_plugin_gm` 是 AstrBot 群管理类插件，Issue/PR 常涉及命令解析、撤回、群管理权限、按群配置、OneBot 适配器兼容、配置 schema 与 README 同步。**高频模式**：权限模型重构（#130/#132）、引用消息触发型命令（#131 群待办）、撤回/STT/头衔/加群申请、装饰字符 QQ 解析（#134/#136）、解禁/解除禁言（#133）、禁言列表查询（#135 读类）、`/解禁 @装饰字符` 误识别（#136）、**"删除既有命令 + 新增/别名对称命令"复合诉求（#138/#139）**、**命令别名/重命名决策类（#138 首次出现）**——出现关键词即套对应模板。

- **"删除功能"类 Issue 不应自动归 medium**：维护者本人纯减法性质的功能裁剪 → low 组合（主动减法+owner 自实施+无横切关注点）。**例外（#138 沉淀）**：纯减法 + 别名化复合诉求，因涉及既有命令删除（breaking-change）+ 用户迁移路径，维持 `medium` 需在优先级章节显式说明"为何不降 low"，给出 4 条决策路径。
- **breaking-change 置信度必须按子诉求分别评估**（#139 沉淀硬约束）：删除部分 0.90-0.95，新增/别名部分 ≤0.30，**禁止给出统一置信度**。
- **owner-driven 复合诉求应强制建议拆分**（#139 沉淀）：维护者合并"删除既有命令 + 新增对称命令"两条独立诉求时，分析必须显式建议拆分为两个独立 Issue 跟踪（除非强约束必须合并），因工作量/风险/决策点完全不同。两条诉求既可拆 Issue 也可合并实施，但合并实施需在标题/优先级章节显式标注。
...
### 命令别名/重命名决策类 Issue 标准模板（#138 案例，**仓库首次沉淀**）

- **触发场景**：维护者或用户反馈命令名不直观/易混淆；中文命令插件常出现 `/取消精华` `/取消群精华` `/取消设精` 多个等价命名共存。
- **关键洞察（#138 沉淀）**：分析前**必须先 grep `_GM_COMMAND_NAMES` + `@filter.command(...)` 装饰器**，建立"命令名→handler"映射，避免误判为"需新增命令"——若已有等价命令，**核心价值是揭示等价性**而非新建。
- **必查项**：① 既有等价命令识别（grep 元组 + 装饰器）② 别名 vs 重命名的取舍（破坏性 vs 兼容性）③ 帮助命令/教程/外部文档影响 ④ `_GM_COMMAND_NAMES` 元组同步 ⑤ CHANGELOG breaking-change 标注 ⑥ 用户迁移路径。
- **标签基线**：`enhancement`(0.95) + `command`(0.95) + `rename`/`alias`(0.75-0.85, 仓库可新建) + `breaking-change`(0.65-0.70, 仅删除部分) + `deprecation`(0.70) + `group-management`(0.85) + `needs-discussion`(0.75-0.85) + `documentation`(0.55-0.65) + `needs-info`(≤0.30 owner-driven) + `onebot`(0.30-0.40 若涉及跨适配器)。
- **标签禁用清单**：`parser`(0) —— 不涉及解析逻辑改动；`recall`(0) —— 非消息撤回（除非确实涉及 `recall_cmd`）。
- **优先级决策路径**（#138 沉淀硬约束）：① 主动减法+owner-driven → 应降 low ② 但涉及既有命令删除（breaking-change）+ 别名决策未确定 → 维持 medium ③ 工作量 1-2 天可控 → 不升 high ④ 跨适配器风险低（v11 标准如 `delete_essence_msg`）→ 维持 medium。
- **可行性分支必显式 A/B/C**：分支A 仅删除（最小化）/ 分支B 删除+保留别名（兼容性优）/ 分支C 完整重命名+迁移指南+配置 schema 调整。
- **工作量估算**：纯删除 0.25-0.5 天（5-8 处修改，含元组/handler/README/CHANGELOG/帮助）；分支 B 0.5-1.5 天；分支 C 1-2.5 天。
- **建议标题**：`[enhancement][medium] 移除 /X 命令并新增 /Y（Y 已被 /Z 覆盖，待确认是否改名）` 或 `移除 /X 并新增 /Y（引用消息触发，与 #Z 对称）`。

### "删除既有命令 + 新增/别名对称命令"复合诉求 Issue 标准模板（#139 案例，**仓库首次出现双动作模式**）

- **触发场景**：维护者一次性提出"删除 /删群公告 + 新增 /取消群待办"等对称命令诉求（如 add/remove、enable/disable、set/unset、bind/unbind、设精/取消设精）。
- **强制拆分/合并建议**（#139 沉淀硬约束）：复合诉求**默认建议拆分为两个独立 Issue 跟踪**，除非：① 工作量 ≤1 天可合并实施 ② 共享同一测试矩阵 ③ 共享同一文档章节。否则应显式写"建议拆分为 #139A（删除）+ #139B（新增）"或"建议合并实施因 Y"。
- **对称命令必查项**（与单方向 #131/#133 模板叠加）：① 对称性（参数、权限、回执是否对称）② 共享 helper 抽取机会 ③ 一方已实现时另一方的依赖反转 ④ **权限语义可能不同**（添加通常宽于撤销：QQ 群待办设置通常群管即可，撤销/取消通常需群主专属）⑤ 跨适配器 API 矩阵 ⑥ UI 异步刷新兜底（API 返回 ok ≠ UI 生效）⑦ 撤回原消息场景。
- **写类对称命令权限不对称风险**（#139 沉淀）：添加侧权限 vs 撤销侧权限必须显式对比（如 `/添加群待办` 群管 vs `/取消群待办` 群主），与 #136 "误解禁 vs 误禁言风险对比"对等。
- **标签基线（breaking-change 复合模板）**：
  ```
  enhancement(0.95) + breaking-change(0.90, 仅删除部分) + deprecation(0.85, 仅删除部分) + 
  command(0.95) + group-management(0.85-0.92) + 
  onebot(0.55-0.70) + compatibility/onebot-extension(0.75-0.85, 仅新增侧) + 
  permission-model/bot-role(0.70-0.80, 涉及权限时) + 
  write-permission(0.80, 撤销侧) + reply/quote-message(0.70-0.85, 引用消息触发) + 
  needs-discussion(0.85, breaking-change 决策点多) + needs-info(≤0.30)
  ```
- **优先级决策路径必显式（4 条）**：① breaking-change+横切调整 → 最低 medium ② owner-driven+已有 #131 模板 → 不升 high ③ 工作量 3-5 天可控+迁移路径明确 → 不升 high ④ 跨适配器风险已识别但有降级路径 → 维持 medium。
- **可行性分支必须显式 A/B/C/D 四档**（#139 沉淀对 #133/#135 三档硬约束的升级）：分支A 仅减法（删除部分）/ 分支B 仅加法（新增对称命令）/ 分支C A+B 合并实施 / **分支D 先 A 后 B 的分阶段方案（推荐）**。禁止只给范围估算不显式分支。
- **工作量**：分支 A 1.5-2.5 天（七处同步：main.py + 元组 + README + 帮助 + CHANGELOG/迁移指南 + metadata + 公告）；分支 B 100-180 行/2-3.5 天（≈ 1.2-1.5× 同方向既有 Issue #131）；分支 C 250-400 行/3-5 天（若 #131 已合并）；分支 D 4-6 天。
- **七处同步清单**（删除既有命令专用，比新增的"五处"更多）：main.py + `_GM_COMMAND_NAMES` + README + 帮助命令 + CHANGELOG/迁移指南 + metadata.yaml 描述清理 + 公告通知。
- **必互引**：减法与对称加法 Issue 应在重复检测章节显式写"非重复但建议合并/分阶段"，与 #131（添加侧）形成 add/cancel 对称关联。
- **建议标题**：`[enhancement][medium] 移除 /删群公告 + 新增 /取消群待办（与 #131 对称）` —— 显式互引同类 Issue。
...
- **重复检测措辞强制模板**：无历史列表时**写"暂未发现"并列出建议检索关键词**，"未检测到重复"/"无重复"/"可能是 #X 的重复"均被明令禁止。每次重复检测输出末加注"⚠️ 措辞核对：是否使用了禁用的'未检测到重复'/'无重复'/'可能是#X的重复'？"。**#134/#135/#136/#138/#139 共五次连续触犯**（#136 反思自己都明确指出"未来反思对重复检测输出自动扫描禁用措辞"，但 #138/#139 仍未根治），**必须升级为反思硬门槛前置自检项**。**未来重复检测章节禁用措辞清单已扩展至包含"可能是#X的重复"**：凡涉及同源但动作不同的 Issue，应改用"与 #X 同源/相关但非重复"。
- **行号溯源前置声明硬约束**（#139 沉淀）：可行性章节引用精确行号（如 `main.py L2414-2433`）时，**必须在章节开头显式声明**："以下行号基于项目记忆与既有 PR 模式推断，PR 实际编写时以最新 main.py 行号为准"。**未声明则视为行号不可信红旗**。
...
- **对称命令扩展必漏标签清单（#139 沉淀）**：分析"对称命令配对类 Issue"时，**必查标签漏给清单**（已沉淀）：`breaking-change`（删除部分 0.90-0.95，新增部分 ≤0.30）、`deprecation`（≥0.75）、`onebot-extension`（替代 `compatibility` 二选一）、`reply`/`quote-message`（0.70-0.85，#131 模板已建立）、`bot-capability`、`permission-model`/`bot-role`（涉及权限时 0.70-0.80）、`write-permission`（撤销侧 0.80）、`changelog`（breaking-change 0.85）。**禁止对子诉求给出统一 breaking-change 置信度**。
- **对称命令扩展"实质新增 vs 别名化"区分（#138 沉淀）**：表面"新增 /X"但实质是为已存在命令添加友好命名入口（语义收敛）——与 #131 "全新能力扩展"在标签上有本质差异，**新增 `rename`/`alias`（0.75-0.85）高权重 + `onebot-extension` 低权重**，新增能力则 `bot-capability` + `onebot-extension` 高权重。
- **可行性分支必须显式 A/B/C 三档**（#133/#135 沉淀硬约束，#139 升级为 A/B/C/D 四档）：禁止只给范围估算不显式分支。**#139 沉淀**：对称命令复合诉求应升级为四档（分支A 仅减法/分支B 仅加法/分支C A+B 合并实施/**分支D 先 A 后 B 分阶段**——推荐方案）。**#138 沉淀**：别名/重命名类分支 C 缺乏独立价值，应合并入分支 B 或删除。
- **写类对称命令权限不对称风险（#139 沉淀，对等 #136 误解禁 vs 误禁言）**：添加侧权限 vs 撤销侧权限必须显式对比（如 `/添加群待办` 群管 vs `/取消群待办` 群主），添加通常宽于撤销，分析时必须显式提及"权限提升/降级影响"。
- **优先级决策路径必须显式列出**（#133 沉淀硬约束）：4 条判定路径显式列出，让后续读者能追溯判定逻辑。同类/同工作量 issue 应同步给出"与 §X 同级，因 Y 原因定 medium"的对比句（#135/#136 沉淀）。
...
- **重复检测的措辞核对已升级为反思硬门槛前置自检项（第 8-9 次触犯）**：禁用清单已扩展至 `"未检测到重复"`/`"无重复"`/`"可能是#X的重复"`/`"暂未发现明显的重复"`四种措辞模式。无历史列表时**写"暂未发现重复 Issue"+ 列出建议检索关键词**。本反思（#139 4 篇汇总）虽已扩展清单但仍未根治，需在反思落笔前扫描替换。
- **owner-driven 双动作/复合诉求的拆分建议（#139 沉淀硬约束升级）**：维护者合并"删除既有命令 + 新增对称命令""功能裁剪 + 别名重命名"等双方向诉求时，分析必须**默认建议拆分为独立 Issue 跟踪**（#139A 减法 + #139B 加法），除非：① 工作量 ≤1 天 ② 共享同一测试矩阵 ③ 共享同一文档章节 ④ 维护者明确要求合并。合并实施需在标题/优先级章节显式标注。
- **项目记忆模板对齐检查是反思硬纪律（第 8-9 次触犯，#138/#139 沉淀）**：分析涉及 #131 群待办 / #135 禁言列表 / #138 别名重命名 / #139 对称命令时，**第一步**必须对照项目记忆模板的"必给标签/必查项"清单逐项勾选。本质问题：把项目记忆模板当"参考"而非"纪律"。**未来改进**：把对照模板逐项核对升级为反思的硬性 checklist，落笔前逐项勾选，缺一不可。
- **"未把项目记忆当强制纪律"是反复触犯的根因（#139 沉淀）**：连续三次反思（#134/#135/#136/#138/#139）都触犯"未逐项核对必给标签"问题，本质是把模板当作建议性参考。**整改措施**：反思落笔前必须先列出本 Issue 对应的项目记忆模板所有必给标签和必查项，再开始写作分析。
- **owner-driven ≠ 无决策**（#135 沉淀）：维护者自实施不等于"无决策"。任何涉及"权限/字段差异/性能"的命令新增都至少有 2-3 个决策点，`needs-discussion` ≥ 0.65。
- **CHANGELOG breaking-change 标注格式模板（#139 沉淀）**：删除既有命令时 CHANGELOG 必须包含"被删命令功能说明 + 替代方案 + 已使用用户迁移路径"三要素，提供标准格式参考。
...
- **小型修复工作量下限**（#134 沉淀，#136/#139 升级）：对"小修复"（<50 行）也要避免低估为 1 天内——装饰字符 QQ 测试矩阵（NapCat/Lagrange/go-cqhttp × 数学字母/手写体/Emoji 风格）至少需要半天构造测试用例。建议小修复工作量下限 1 天，包含最小适配器验证。**#139 沉淀传染性测试成本**：13+ 调用点扫描 + 适配器差异 + 边界场景构造，建议传染性测试成本 +1 天而非 +0.5 天。
