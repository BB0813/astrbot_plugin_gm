...
- **增量审查失败的处理**：
  - 工具输出验证失败 → 拒绝提交，标记 "manual review required"
  - None/10 评分应触发告警，禁止产生无效审查决策
...
### 4.1 命令名验证
```
引用任何命令名之前 → 必须用 grep 验证（先搜索代码，再引用）
禁止：未经代码验证直接引用命令名
```
**错误示例**：`/清用户历史` vs `/清空历史记录` — 命令名必须与代码一致
...
needs-discussion 置信度偏高 → 实际方案明确时应降低至 0.3-0.5
```
...
4. **增量审查** — 合并提交前验证原 PR 是否已审查；增量包含合并提交时策略升级
5. **Issue 分析命令名** — 必须先 grep 验证再引用，禁止凭记忆引用
6. **breaking-change 评分** — 必须附带具体处置建议，不止于评分
7. **配置项移除 breaking-change 评分** — 建议 0.90-0.95，需 CHANGELOG 同步
8. **标签置信度精细化** — needs-discussion 0.75 仅适用于真正需要讨论的议题
9. **审查工具验证失败** — 产生 None/10 应拒绝提交，标记 manual review required
