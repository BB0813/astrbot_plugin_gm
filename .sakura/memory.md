...
仓库 `mjy1113451/astrbot_plugin_gm` 是 AstrBot 群管理类插件，Issue/PR 常涉及命令解析、撤回、群管理权限、按群配置、OneBot 适配器兼容、配置 schema 与 README 同步。**高频模式**：权限模型重构（#130/#132）、引用消息触发型命令（#131 群待办）、撤回/STT/头衔/加群申请、装饰字符 QQ 解析（#134/#136 第二次触发高频）、解禁/解除禁言（#133）、禁言列表查询（#135 读类）、**/解禁 @装饰字符 误识别（#136）**、**命令别名/重命名决策（#138 新）**、**删除+新增复合诉求（#139 新）**——出现关键词即套对应模板。
...
### 装饰字符 QQ / 视觉欺骗型用户名 Issue 标准分析模板（#134/#136 案例，**高频解析类 bug 模式，第 2 次触发 #136**）
...
- **必查项**：① `_extract_at_qq` 是否 NFKC/NFKD 归一化 ② OneBot 适配器是否对装饰字符 QQ 拒绝/截断 ③ 群号是否也被装饰字符污染 ④ 是否需在输入层加"QQ 必须是纯数字"硬校验 ⑤ 装饰字符 QQ 跨适配器命令成功率矩阵（NapCat/Lagrange/go-cqhttp × 数学字母/手写体/Emoji 风格）⑥ 完整执行日志 ⑦ `_extract_at_qq` 解析路径（必须按 segment/user_id，不按空格 split）⑧ 同类命令传染性（`/禁言` `/踢人` `/设管` `/取管` 都共享 `_extract_at_qq`，修复需同步）⑨ 多 `@` 取首个 vs 拒绝多目标的语义边界 ⑩ **OneBot v11 协议层是否已被 NapCat/Lagrange 预处理**（部分实现会把无法识别的 at 直接丢弃，会改变修复优先级）⑩ **`raw["message"]` 中装饰字符 AT 段字段差异**（v11 标准是 `{type:"at", data:{qq:"xxx"}}`，扩展可能用 `user_id`）。
...
- **横向对比矩阵（#136 沉淀）**：所有"装饰字符 QQ 解析"模式 Issue（#134/#136 后续）建立对比：Issue 号 / 触发命令 / 副作用方向（误禁言 vs 误解禁 vs 误踢人）/ 优先级 / 备注。横向对比让读者看清"为何同样模式优先级有差异"。
- **修复建议**：在 `_extract_at_qq` 顶部加白名单 `\d{5,12}` 强校验 + 错误提示"目标 QQ 格式不正确"。NFKC 只能作为辅助。
- **必给标签清单（必须逐项勾选，#136 曾遗漏 at-parse/unmute）**：`bug`(0.95) + `command`(0.95) + `parser`(0.85) + `at-parse`/`at-extract`(0.85, 仓库可新建) + `group-management`(0.80) + `onebot`(0.35-0.50) + `compatibility`(0.35-0.50) + `mute`/`unmute`/`mute-action`(0.85, 仓库可新建) + `unicode`/`unicode-normalization`/`decorative-unicode`(仓库可新建) + `input-validation`(仓库可新建) + `needs-info`(≤0.30 owner-driven 模板已沉淀)。
...
### "删除功能" / 纯减法裁剪类 Issue 标准分析模板（#138 沉淀，**主动减法+owner-driven+无横切关注点 → low 组合**）

- **触发场景**：维护者本人要求删除既有命令/配置项/辅助函数，工作量小、影响面窄、不涉及权限模型或横切关注点。
- **必查项**：① 是否已有等价替代命令（避免误判为新功能需求）② 既有命令使用频率与用户迁移成本 ③ 帮助命令/教程/外部文档影响 ④ `_GM_COMMAND_NAMES` 元组同步 ⑤ CHANGELOG breaking-change 标注 ⑥ 调用点清空（main.py 全文搜索是否仅有一处定义+一处注册）⑦ helper 是否还有其他调用路径。
- **优先级决策路径（#138 沉淀）**：① 主动减法+owner-driven+无横切关注点 → 应降 low ② 但涉及既有命令删除（breaking-change）+ 用户迁移 → 维持 medium ③ 工作量可控（≤1-2 天） → 不升 high ④ 跨适配器风险低（v11 标准）→ 维持 medium/降 low。
- **标签基线（#138 沉淀）**：`enhancement`(0.95) + `command`(0.95) + `group-management`(0.85) + `breaking-change`(0.65-0.70) + `deprecation`(≥0.65) + `needs-discussion`(0.80+) + `needs-info`(≤0.30 owner-driven) + `documentation`(0.55-0.65) + `onebot`(0.30-0.40)。**不**应给：`parser`(非解析类)、`recall`(非撤回类)。
- **工作量下限**：纯删除极简（~5-8 处修改，半小时可完成），下限可降至 0.25-0.5 天；含迁移指南+边界测试则 1.5-2.5 天。
- **删除既有命令的 9 项反向清单**（#138 沉淀）：main.py handler + helper + 元组注册 + README 帮助 + CHANGELOG breaking-change + 帮助命令文案 + metadata.yaml 描述 + 配置 schema 清理 + 公告（迁移指南）。

### "命令别名/重命名决策"类 Issue 标准分析模板（#138 新沉淀）

- **触发场景**：维护者或用户反馈命令名不直观/易混淆；或功能等价但命名不同（中文 bot 高频陷阱，如 `/取消精华` `/取消群精华` `/取消设精` 多名共存）。
- **必查项**：① 既有等价命令识别（避免误判为新功能需求）② 别名 vs 重命名 vs 共存的取舍 ③ 帮助命令/教程/外部文档影响 ④ `_GM_COMMAND_NAMES` 元组同步 ⑤ CHANGELOG breaking-change 标注 ⑥ 多 `@`/多参数场景是否被新名覆盖 ⑦ 用户迁移路径（旧名是否保留为别名）。
- **标签基线（#138 沉淀）**：`enhancement`(0.95) + `command`(0.95) + `rename`/`alias`(0.75-0.85, 仓库可新建) + `group-management`(0.85) + `breaking-change`(0.65-0.70) + `needs-discussion`(0.75-0.85) + `deprecation`(0.65-0.75 若旧名删除) + `documentation`(0.55-0.65) + `onebot`(0.30-0.40)。**不**应给：`parser`(非解析类)、`recall`(非撤回类)。
- **可行性分支必须显式**：分支A 最小（仅重命名，不保留别名）/ 分支B 完整（重命名+保留别名+迁移指南）/ 分支C 完整版+配置 schema 调整。**分支 C 缺乏独立价值时应合并入分支 B**（#138 反思警示）。
- **优先级**：参照"删除功能类"决策路径，纯减法+owner-driven 应优先降 low；改名+迁移成本可控时维持 medium。
- **"语义收敛型 vs 全新能力扩展"标签区分**（#138 沉淀）：全新能力扩展给 `bot-capability` + `onebot-extension` 高权重；语义收敛（仅为已有功能加友好命名）给 `rename`/`alias` 高权重、`onebot-extension` 低权重。
- **核心洞察**：本类 Issue 价值往往不在"重命名实施"，而在揭示"既有等价命令已覆盖新名诉求"，避免无意义的"新建"工作。建议在标题、优先级、可行性三维度都体现这一发现。

### "删除+新增对称命令"复合诉求 Issue 标准分析模板（#139 新沉淀）

- **触发场景**：维护者合并"删除既有命令 + 新增对称命令"两条独立诉求（如删除 `/删群公告` + 新增 `/取消群待办`；或 `/添加群待办` ↔ `/取消群待办` 对称）。
- **必查项**：① 两条诉求是否真正对称（参数、权限、回执）② 共享 helper 抽取机会 ③ 一方已实现时另一方的依赖反转 ④ **权限语义可能不同**（添加通常宽于撤销——如添加群待办可能允许群管，撤销通常需群主）⑤ 跨适配器 API 矩阵 ⑥ 撤回原消息场景 ⑦ UI 异步刷新兜底（API 返回 ok ≠ UI 撤销生效）⑧ **是否需状态回读**（如调用 `get_group_todo_list` 校验撤销结果）。
- **必须拆分或显式标注**（#139 沉淀硬约束）：两条诉求的工作量、风险、决策点完全不同，分析必须显式建议"拆分为两个独立 Issue 跟踪"或"合并实施并显式标注"。混合 PR 会导致审查困难。
- **breaking-change 置信度按子诉求分别评估**（#139 沉淀硬约束）：删除部分 0.95，新增部分 ≤0.30，**禁止给出统一置信度**。
- **deprecation 标签必须必给**（#139 沉淀）：删除既有命令/配置项时 `deprecation` ≥0.85 是硬约束。
- **七处同步清单对"删除既有命令"特别适用**（#139 沉淀）：删除需清理的扩散点比新增更多（README/帮助/CHANGELOG/metadata/schema/元组注册/公告），可行性章节强制列**七处**而非五处。
- **标签基线（#139 沉淀）**：`enhancement`(0.95) + `breaking-change`(删除 0.95/新增 ≤0.30) + `command`(0.95) + `group-management`(0.85-0.92) + `onebot`(0.55-0.70) + `compatibility`/`onebot-extension`(0.75-0.85) + `permission-model`(0.70 权限不对称) + `quote-message`/`reply`(0.85, 仓库可新建) + `deprecation`(≥0.85) + `configuration`(0.65-0.75) + `needs-discussion`(0.75-0.85) + `needs-info`(≤0.30) + `documentation`(≥0.75 涉及 CHANGELOG breaking-change)。
- **工作量下限**（#139 沉淀）：删除部分（含七处同步）至少 0.5-1 天而非半小时；分支 B 完整实施 200-350 行/3-5 天；分支 C 合并实施 250-400 行/3-5 天（若 #131 已合并）。
- **写类对称命令权限不对称风险**（#139 沉淀）：分析时必须显式对比"添加侧权限 vs 撤销侧权限"，与 #136 沉淀的"误解禁 vs 误禁言风险对比"对等。
- **重复检测措辞**（#139 反思）：#139 与 #131 形成 add/cancel 对称命令，应在重复检测章节显式标注"与 #131 形成对称命令"，而非仅在"关联 Issue 互引"中提及。
...
- **跨 Issue 互引规范**（#135 沉淀）：同类/同工作量 issue 应在优先级章节显式对比说明一致性（如 #135 与 #131 群待办对比），给出"与 §X 同级，因 Y 原因定 medium"的对比句。
...
- **校验失败反思的硬约束升级为"前置自检门槛"**（#135 沉淀）：反思落笔前必须先扫描反模式禁用清单（"无法评估"/"未检测到重复"/"无建议"/"可能是#X的重复"/"无重复"），任一命中则视为实质性判断已被连带退化的红旗信号。
- **重复检测措辞强制模板**：无历史列表时**写"暂未发现"并列出建议检索关键词**，"未检测到重复"/"无重复"/"可能是 #X 的重复"均被明令禁止。每次重复检测输出末加注"⚠️ 措辞核对：是否使用了禁用的'未检测到重复'/'无重复'/'可能是#X的重复'？"。**#134/#135/#136 三次连续触犯**（#136 还有"可能是 #128 的重复"无依据盲指），必须作为反思 checklist 必检项。**未来反思对重复检测输出自动扫描禁用措辞**，固化反模式检查。
...
- **同根因 issue 优先级显式对比句**（#135/#136 沉淀）：同类/同工作量 issue 应在优先级章节显式对比说明一致性，给出"与 §X 同级，因 Y 原因定 medium"的对比句，避免读者横向对比产生质疑。
- **项目记忆模板对齐检查**（#136 沉淀）：分析涉及装饰字符 QQ / @解析 / 禁言类 Issue 时，**第一步**必须对照项目记忆对应模板的"必给标签"/"必查项"清单逐项勾选，不允许遗漏。这是 #134/#135/#136 三次触犯后的强制应用纪律。
...
- **"删除+新增"复合诉求 Issue 必须强制拆分建议**（#139 沉淀硬约束）：维护者合并两条独立诉求时，分析必须显式建议拆分为两个独立 Issue 跟踪，除非有强约束必须合并实施。两条诉求的工作量、风险、决策点完全不同，混合 PR 会导致审查困难。
- **breaking-change 置信度按子诉求分别评估**（#139 沉淀硬约束）：删除部分 0.95，新增部分 ≤0.30，禁止给出统一置信度。**对称命令扩展类 Issue 强制要求**。
- **deprecation 标签必须必给**（#138/#139 沉淀）：删除既有命令/配置项时 `deprecation` ≥0.85 是硬约束（#130/#132 模板已固化，本次 #139 仍触犯遗漏）。
- **行号引用必须前置声明可溯源状态**（#139 沉淀）：精确行号若无"已读取 main.py 验证"，应在可行性章节开头声明"基于项目记忆与既有 PR 模式推断，PR 实际编写时以最新 main.py 行号为准"。
- **纯减法/裁剪类 Issue 必须优先考虑 `low`**（#138 沉淀）：owner-driven + 主动减法 + 无横切关注点 → 应降 low 而非默认沿用 medium；除非涉及 breaking-change + 用户迁移才维持 medium。
- **同类 Issue 决策树显式列出**（#138 沉淀）：若 Issue 含"作者意图二义性"（如命令命名歧义），应在分支选择章节明确"如果作者回复 (a) → 走分支B/C；如果作者回复 (b) → 走分支A；如果作者不回复 → 默认分支A（最保守）"。
- **分支 C 缺乏独立价值时应合并入分支 B**（#138 反思警示）：当分支 C 仅在分支 B 基础上加 schema 调整但已在 helper 内部实现 fallback 路径时，不应作为独立分支，避免工作量虚高。
- **"语义收敛型" vs "全新能力扩展"标签区分**（#138 沉淀）：仅为已有功能加友好命名（语义收敛）给 `rename`/`alias` 高权重、`onebot-extension` 低权重；全新能力扩展给 `bot-capability` + `onebot-extension` 高权重。
