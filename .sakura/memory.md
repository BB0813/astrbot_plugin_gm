...
- **必查项 10 条**：①事件钩子获取（AstrBot `group_decrease` / 适配器层 / 轮询）②副作用边界（清自己/清被踢者/清缓存）③**回执一致性**（B 部分失败如何回执 A 成功状态）④**通知风暴**（撤回通知影响其他群成员）⑤**限流策略**（B 高成本动作如何分批，全局 sleep 会阻塞事件循环）⑥配置粒度（可关闭/可调上限/按群覆盖）⑦**权限双校验**（A/B 各自权限对齐 + bot 自身权限边界）⑧时间窗约束（OneBot 2 分钟）⑨跨适配器差异矩阵 ⑩**去重**（本地缓存与服务端历史可能产生重复 message_id）。
- **⚠️ 隐私合规必查（#145 强化 P0）**：涉及用户数据删除/修改时，**必须给 `privacy` 标签（0.75-0.85）**，并评估：①审计日志（操作者+消息ID+时间戳）②用户通知（私聊告知被清除范围）③撤回通知抑制（避免群内风暴）④失败回滚（部分失败时回执 N/M）。
- **⚠️ 外部仓库验证与标签区分（#143 强化）**：
  ...
  - **必须先访问外部仓库验证实现路径**，可行性分支需标注"假设外部仓库路径可借鉴"，工作量估算在验证前不可靠
- **⚠️ 默认值决策 P0（#145 新增）**：默认 false → 维持 medium；**默认 true → 升 high**（silent behavior change）。优先级章节必须显式说明默认值决策。
- **必给标签硬清单**：⚠️ 动作联动型 11 项逐项核对（#145 第 N+2 次漏标教训）：
  - `enhancement`(0.95) + `command`(0.85-0.95) + `configuration`(0.80-0.90) + `group-management`(0.85-0.92) + `permission-model`(0.80-0.85) + `kick`(0.85) + `message-history`(0.75-0.85, 新建) + `auto-action`(0.70-0.80, 新建) + `privacy`(0.75-0.85, 新建 P0) + `onebot`(0.55-0.90) + `onebot-extension`(0.55-0.65) + `compatibility`(0.55-0.75) + `external-reference`(0.85, 参考而非合入) + `breaking-change`(0.40-0.65) + `needs-discussion`(0.70-0.85) + `needs-info`(≤0.30, 双校准对冲)
- **⚠️ 必删误标标签（#138/#143 教训）**：`parser` 仅在装饰字符/`_extract_at_qq`/`_get_reply_id` 场景给；`recall` 仅在撤回命令场景给。
- **可行性分支细化要求**：分支 B/C 必须显式标注以下 4 项硬约束行数（#145 第 N+2 次漏标教训）：
  - ① 撤回时限逐条检测 helper（约 20-30 行）
  - ② 撤回通知抑制（约 10-20 行）
  - ③ message_id 去重 set（约 5-10 行）
  - ④ 三类回执计数（成功 N / 跳过 X / 失败 Y）（约 10-15 行）
  - 分支 B 修正为 250-400 行/3.5-5.5 天；分支 C 修正为 400-600 行/6-9 天
- **"参考外部仓库"vs"外部合入"标签区分（#143 沉淀）**：`external-reference` + `external-repo` 用于参考借鉴；`merge-request` + `license-check` 用于真正合入。**不可混用**。
- **外部仓库参考类前置检查清单**：①许可证兼容 ②依赖兼容（aiohttp/httpx/websocket-client 版本）③OneBot 标准 vs 私有扩展 API 使用清单 ④维护责任归属 ⑤配置 schema 扩展冲突 ⑥**必须先访问外部仓库验证实现路径**。
- **优先级决策路径 5 条（#142 升级，含默认值）**：①②③④ 同前 + ⑤**默认值决策（默认 false → 维持；默认 true → 升 high/silent behavior change）**。
- **可行性分支**：A 80-150 行/1-2 天 / B 150-250 行/2-3 天 / C 250-400 行/3-5 天 / D 分阶段 4-6 天。
- **可观测性回执**：成功 N/M、跳过 X 条因时间窗、失败 Y 条因限流 的分类回执。
- **红旗**："清历史/清记录"涉及 GDPR/个人信息删除合规风险（主流平台 Bot API 通常不允许运营方清空他人消息），必须显式评估滥用/合规/不可逆性。
- **#142/#143/#145 三者关系索引**（#145 新增）：
  - #142：动作 A → 动作 B（A=踢人, B=撤回）——**撤回方向**
  - #143：动作 A → 动作 B（A=踢人, B=清历史）——**清历史方向，与 #145 几乎一致，建议合并**
  - 动作联动型 Issue 之间的高频重叠需显式互引，重复检测前置过滤：主分类不同→0.3；主分类同但 API 不同→0.30
...
## 反模式（P0 硬约束，#142/#143 第 9-10 次触发）

- **⚠️ 校验失败短路器（P0，第 N+1 次触发 #143）**：字段校验失败**仅修复字段输出**，**不得整体降级为"无法评估"/`other`/空标签/无建议**。**Pre-check**：含"无建议/无法评估/空/未检测到重复"前必先确认是否源于校验失败——校验失败应**局部修复**。**实质性判断不得连带退化**。**#143 整体降级**。**"#143 恢复模式"**：下次遇整体降级，**先恢复实质性判断再修复字段输出**。
- **⚠️ 必给标签逐项核对 P0（#145 第 N+2 次漏标）**：常见遗漏 `at-parse`/`unmute`（#136）、`onebot-extension`/`reply`（#139）、`bot-role`/`notification`（#140）、`group-management`/`needs-info`/`compatibility`/`external-reference`（#142/#143）、**`privacy`（#145）**。**`parser` 仅在装饰字符/`_extract_at_qq`/`_get_reply_id` 场景给**（#138/#143 多次误标）；**`recall` 仅在撤回命令场景给**。
- **⚠️ 可行性分支细化 P0（#145）**：分支 B/C 必须显式标注 4 项硬约束行数：①撤回时限 ②通知风暴抑制 ③去重 set ④三类回执计数。分支 B 修正为 250-400 行；分支 C 修正为 400-600 行。
