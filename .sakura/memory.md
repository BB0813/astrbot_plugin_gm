...
- **OneBot v11 协议层装饰字符 AT 段字段差异（#136 沉淀）**：v11 标准是 `{type:"at", data:{qq:"xxx"}}`，部分扩展用 `{type:"at", data:{user_id:"xxx"}}`，字段名差异会污染修复方案。NapCat/Lagrange 对装饰字符 QQ 在协议层是否被预处理（部分实现直接丢弃不可识别 AT）会改变修复优先级（若协议层已拦截则需在适配器层处理而非插件层）。
- **误解禁 vs 误禁言风险对比（#136 沉淀）**：解禁类装饰字符 QQ 误识别比禁言类更敏感——前者可能误解禁一个本应继续禁言的用户（如第一个 `@用户` 恰好是另一被禁言成员）；若 API 调用本身是无副作用空操作则实际后果较低。优先级章节应显式做横向对比说明为何不升 high。
...
- **结构化输出校验失败 ≠ 信息不足**：看到 `expected <SUGGESTED_TITLE>` 等字段校验错误时，应**仅修复字段输出**，**不得把所有判断都退化为"无法评估"/`other`/空标签/无建议**。**字段校验失败仅触发输出修复，不得影响实质性判断**。**Pre-check 规则**：在任何字段出现"无建议/无法评估/空/未检测到重复"前，必须先确认是否源于校验失败——校验失败应**局部修复**而非**整体降级**。**#135 已第 5 次整体降级，校验失败短路器必须升级为前置自检门槛**。
- **校验失败短路器（必须固化，已第 5-6 次触发 #131/#132/#133/#134/#135/#136）**：字段校验错误仅触发输出修复，**实质性判断（分类/优先级/标签/可行性/标题、重复检测）维持原始判断，不得连带退化**为 `other`/空标签/无建议。**反思指令硬约束**：作为反思的**第一优先级自检项**，落笔前必须扫描。**#135/#136 沉淀新增**：反思模板"反模式自检"环节必须固化 6 条硬约束（字段校验失败仅局部修复 / 标题/标签/可行性/重复检测不可写禁用措辞 / 实质性判断不得连带退化 / 重复检测末必须加注"⚠️ 措辞核对"）。
- **重复检测措辞强制模板**：无历史列表时**写"暂未发现"并列出建议检索关键词**，"未检测到重复"/"无重复"/"可能是 #X 的重复"均被明令禁止。每次重复检测输出末加注"⚠️ 措辞核对：是否使用了禁用的'未检测到重复'/'无重复'/'可能是#X的重复'？"。**#134/#135/#136 三次连续触犯**，必须作为反思 checklist 必检项。**#136 沉淀新增**：重复检测输出自动扫描禁用措辞，违规必须改写为"与 #X 同根因模式，建议合并修复"或"暂未发现重复，建议检索关键词..."。
...
- **读权限 vs 写权限区分**（#135 沉淀）：本仓库 issue 多关注"谁能执行"（写权限），但"群管理列表查询"涉及"谁能查看"（读权限）。仓库标签体系若仅有 `permission`，应细化为 `write-permission` / `read-permission` / `viewer-role`。读权限可能被滥用为侦察工具，需评估可见范围。**#135/#136 沉淀新增**：读类命令必给标签 `read-permission`/`viewer-role`(0.70-0.85) + `onebot-extension`/`bot-capability`（非 v11 标准字段专用）。
- **跨适配器读取类群成员字段差异矩阵（#135/#136 沉淀）**：`role`、`shut_up_timestamp`（NapCat/go-cqhttp）、`ban_expire_time`（Lagrange）、`mute_end_time`（部分实现）、`last_sent_time`、`join_time`、`level`、`special_title` 的跨实现差异矩阵已正式建立——所有"读群成员 X 字段"类 issue 必须先列该字段对照表。**#135 沉淀执行要点**：任何查询"群成员 X 字段"的 issue 都应主动列出 fallback 表。
- **建议仓库新增标签子体系（#135 沉淀）**：`mute`/`ban`/`ban-list`/`unmute`/`mute-action`/`read-permission`/`viewer-role`——与 `title`、`recall`、`vote` 并列，便于禁言/读权限类 issue 检索。`at-parse`/`at-extract` 与 `decorative-unicode`/`unicode-normalization`/`input-validation` 应与装饰字符 QQ 模板同步建立。
...
- **小型修复工作量下限**（#134 沉淀）：对"小修复"（<50 行）也要避免低估为 1 天内——装饰字符 QQ 测试矩阵（NapCat/Lagrange/go-cqhttp × 数学字母/手写体/Emoji 风格）至少需要半天构造测试用例。建议小修复工作量下限 1 天，包含最小适配器验证。**#136 沉淀修正传染性测试成本**：13+ 调用点扫描 + 跨命令回归 + 适配器差异测试 + 边界场景（混装饰 + 半数字 + 中间夹装饰）= +1 天而非 +0.5 天。
- **同根因 issue 差异化纪律（#136 沉淀）**：即使根因相同（同 `_extract_at_qq` 未做装饰字符校验），也需在优先级、传染性测试、风险点三方面做差异化分析；同根因应保持优先级一致（#134 与 #136 都是 medium），避免读者横向对比产生质疑；建议在优先级章节显式加"与 #X 保持一致"+ 风险对比。
- **同根因重复检测置信度上限（#136 沉淀）**：主分类+API 都相同时（如 #134 与 #136 都是 bug + at 段解析），置信度上限 0.75-0.85；若症状/触发命令差异明显（如 #134 踢人 vs #136 解禁），置信度上限 0.75，措辞改为"高度疑似重复，建议合并或互相引用"而非 `duplicate` 标签。
- **模板对齐纪律（#136 沉淀强制应用）**：分析涉及装饰字符 QQ / @解析 / 禁言类 Issue 时，**第一步**必须对照项目记忆对应模板的"必给标签"/"必查项"清单逐项勾选，不允许遗漏（#136 漏给 `at-parse`、`unmute`）。这是对装饰字符 QQ 模板的强制应用纪律。
- **同类 Issue 横向对比矩阵（#135 沉淀）**：所有同类模板 issue 应建立对比矩阵（Issue 号 / 触发命令 / 副作用方向 / 优先级 / 备注），让读者一眼看清"为何同样模式优先级有差异"。同类/同工作量 issue 应在优先级章节显式写"与 §X 同级，因 Y 原因定 medium"的对比句。
- **决策路径显式化对同类 Issue 的特殊适用（#136 沉淀）**：同根因 issue 优先级决策路径必须显式列出 4 条判定路径（如 #136：①核心解禁命令解析错误 high 倾向 ②但仅装饰字符触发降一档 ③单群单次未扩散降一档 ④已有 #134 同根因模板可复用不算新风险 → medium），让后续读者能横向对比一致性。
- **多 `@` 目标命令兼容性边界（#136 沉淀）**：必须确认仓库 `/禁言` `/解禁` `/踢人` 等命令是否支持多用户（`/解禁 @A @B @C`）。若支持，"取第一个合法 `@` 目标"会破坏多用户场景；若不支持应明确文档并彻底拒绝多 `@`。修复策略必须明确处理该边界。
